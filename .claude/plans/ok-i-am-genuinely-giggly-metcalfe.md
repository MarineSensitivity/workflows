...
1. **Resolution / storage** — single **global 0.05° cell index over the FULL globe** (land + freshwater +
   ocean, **not** ocean-masked, so a bird's terrestrial range or a salmonid's freshwater range counts
   toward total range/endemism). One cell-id COG, longitude standardized to **[-180,180]**. Per-species
   distributions live in **dataset-partitioned Parquet on S3**, queried lazily by DuckDB/titiler; the
   **same cell index** is reused by the (US-only) score/aggregate layers.
...
3. **Validity flags** — replace the single `is_ok` with three booleans on `taxon`:
   ...
   - **`is_valid_pra`** = `is_valid_usa` **and** has ≥1 merged cell inside the 20 BOEM Program Areas.
     ≈ 9,230.
4. **Range size / endemism** — computed for every `is_valid_global` species as the **total range** (Σ
   occupied-cell `area_km2` across land + freshwater + ocean, so a bird's terrestrial extent and a
   salmonid's freshwater extent are included); exposed as a taxon attribute + in the species app/report.
   Not wired into scoring in v8 (rarity-weighted scoring is a future option).

## Architecture at a glance
```
          GLOBAL 0.05° cell index  (r_cellid_global.tif COG, [-180,180], FULL globe land+freshwater+ocean, ~25.9M cells)
...
## Phase 1 — Global 0.05° cell grid + COG (full globe, not ocean-masked)
**Files:** `workflows/ingest_aquamaps_to_sdm_duckdb.qmd` (grid build `~509-577`), `workflows/libs/am_functions.R`.
- New `create_grid_global` chunk: build a **full-globe** 0.05° grid (7200×3600 = 25.92M cells, every pixel
  a cell — **no ocean/land mask**, so terrestrial bird ranges and freshwater salmonid ranges are
  representable), assign global `cell_id`, write **COG in [-180,180]** to `r_cellid_global.tif`. Record
  `area_km2` per cell via `terra::cellSize` (varies by latitude) and boolean `in_usa` (within
  `ply_boem-usa.gpkg`, the US **marine/to-shore** study area used for scoring) / `in_pra` (within
  `ply_programareas_2026`) on `cell`.
- This **replaces** the US-only `r_bio-oracle_planarea.tif` as the canonical index; US scoring cells become
  the `in_usa` subset of the global grid (cell_ids are renumbered — expected for a new version).

**Verify:** `cell` rowcount = 25,920,000 (full 7200×3600 globe); `sum(in_usa)` ≈ prior 661k order; COG
`/debug/cog` reports [-180,-90,180,90]; a known land lon/lat and a known ocean lon/lat both map to valid
`cell_id`s.
...
**Verify:** per-dataset Parquet species/row counts; an Indo-Pacific reef fish has global cells and zero
`in_usa` cells; a migratory bird shows **terrestrial** cells and a salmonid shows **freshwater** cells
(confirming the full-globe grid captures non-marine range).
...
- **Full-globe 0.05° volume**: 25.92M cells (vs ~17M if ocean-masked) and per-species distributions across
  land+freshwater+ocean make the merged global Parquet large; measure before the full build. Consider a
  separate `sdm_global` store so the US scoring DB stays lean.
...
- Counts: `is_valid_global` ≫ `is_valid_usa` (~16k) > `is_valid_pra` (~9,230); total range size sane
  (cosmopolitan ≫ endemic; a bird's terrestrial range and a salmonid's freshwater range are counted).
- Species app renders any global species worldwide at 0.05° via tiles (incl. terrestrial/freshwater
  extent); US species unaffected.
