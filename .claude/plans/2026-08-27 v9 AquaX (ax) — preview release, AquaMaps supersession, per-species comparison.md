...
| D6 | COG representations | mirror `am`: **`native/ax_native/{id}.tif`** = band 1 as delivered (Float32 0–1000, cropped to data bbox, AUC/TSS/cutoff as GDAL metadata) and **`native/ax/{id}.tif`** = the surface as ingested (INT1U 1–100 from the Parquet). *Confirmed 2026-08-27 (Ben, "option 1"):* for an on-grid dataset these are **as delivered vs as ingested** — same topology, the difference is ÷10 + integer + the ≥1 threshold (≈ half of a model's pixels) — NOT original vs interpolated; `dataset.on_grid` (front-matter `on_grid: true`) relabels the app toggle *Delivered / As ingested* | keeps the ingest threshold inspectable on the input itself; the alternative (alias the native COG for both) was rejected |
