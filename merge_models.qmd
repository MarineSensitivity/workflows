...
msens::copy_atlas_parquet(con, glue(
  ...
  dir_parts, partition_by = "ms_merge_key")
# VIEW over the partitioned input — EVERY query below filters ms_merge_key so reads are pruned
dbExecute(con, glue("CREATE OR REPLACE VIEW mc_parts AS
  SELECT ms_merge_key, ds_key, cell_id, val
  FROM read_parquet('{dir_parts}/**/*.parquet', hive_partitioning = true)"))
all_keys <- dbGetQuery(con, glue("SELECT DISTINCT ms_merge_key
  FROM read_parquet('{dir_parts}/**/*.parquet', hive_partitioning = true)"))$ms_merge_key
log_info("partitioned merge input: {length(all_keys)} taxa by ms_merge_key; US trim {n_us} in_usa cells; memory_limit={mem_gb}GB")
```
...
# whole-range (global) multiplicative turtle merge, reading ONLY the 6 turtle taxa's partitions
# (ms_merge_key IN (...) → partition-pruned, tiny). er/ch/suit all come from the one pruned slice.
dbExecute(con, glue("CREATE OR REPLACE TABLE mc_turtle AS
  WITH t AS (SELECT ms_merge_key, ds_key, cell_id, val FROM mc_parts WHERE ms_merge_key IN ({tkeys_sql})),
...
nonturtle <- setdiff(all_keys, turtle_keys)
batches   <- split(nonturtle, ceiling(seq_along(nonturtle) / 150))
log_info("merging {length(nonturtle)} non-turtle taxa in {length(batches)} pruned batches")
...
for (i in seq_along(batches)) {
  bk <- paste(sprintf("'%s'", batches[[i]]), collapse = ",")
  dbExecute(con, glue("CREATE OR REPLACE TABLE b AS
    SELECT ms_merge_key, ds_key, cell_id, val FROM mc_parts WHERE ms_merge_key IN ({bk})"))
