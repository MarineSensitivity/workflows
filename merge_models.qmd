...
#| eval: !expr (do_merge_all || do_merge_turtles)
...
# v7: program-area membership is NO LONGER a taxon-level gate on `is_ok`.
# `is_ok` now means taxonomic/data validity + has a merged distribution somewhere
# in the (study-area) grid. Whether a species falls within a BOEM Program Area is a
# spatial/zone query handled downstream (zone_cell / zone_taxon), not a taxon flag.
# Removing this gate restores the full valid set (~16,158) from the previous
# ~9,424; within-Program-Area scores are unaffected because the restored species
# have no cells inside the Program Areas. See dev/build_v7.R and the v7 plan.
# (Previously: is_ok <- is_ok & taxon_id %in% taxa_in_pra.)
...
# require an actual mapped distribution ----
# a valid species must have >=1 cell in model_cell; a few taxa carry a stale
# mdl_seq with no model_cell rows (empty merged model) and must be excluded so
# the is_ok count matches the species actually mapped (zone_taxon / apps).
dbExecute(
  con_sdm,
  "UPDATE taxon SET is_ok = FALSE
    WHERE is_ok AND mdl_seq NOT IN (
      SELECT DISTINCT mdl_seq FROM model_cell WHERE mdl_seq IS NOT NULL)"
)
