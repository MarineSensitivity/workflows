...
- **v9: AquaX (`ax`) supersedes AquaMaps (`am`) per taxon, inside the AquaX mask only.** Before
  ...
  run (ax registered, nothing superseded) whose `pra_score_delta` against v8 must be cor 1.000.
...
if (n_sup > 0) {
  leak <- dbGetQuery(con, "
    ...
    FROM mc_parts p
    WHERE p.ds_key = 'am'
      AND p.mkey_id IN (SELECT mkey_id FROM mkey_map WHERE ms_merge_key IN (SELECT ms_merge_key FROM supersede))
      AND p.cell_id IN (SELECT cell_id FROM ax_mask)")
