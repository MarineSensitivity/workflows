...
rng_shps           <- list.files(dir_iucn_raw, pattern = "\\.shp$", recursive = T, full.names = T)
...
for (i in 1:length(rng_shps)){ # i = 4 # EELS.shp # i = 9 # MAMMALS_MARINE_ONLY.shp
  rng_shp  <- rng_shps[i]  
  ...
  if (file.exists(grp_gpkg)){
    ...
    a <- read_sf(rng_shp)  # st_drop_geometry(a) |> View()
    ...
    if (length(flds_notreq) > 0)
      log_error("| ERROR | {grp} | missing required fields: {paste(flds_notreq, collapse = ', ')}")
    ...
    if (any(!st_is_valid(a))){
      log_warn("| WARN | {grp} | repairing invalid geometries before summarizing")
      a <- st_make_valid(a)
    }
    if (any(!st_is_valid(a))){
      ...
      invalid_sci <- a %>% 
        ...
        pull(sci_name) |> 
        unique()
      log_warn("| SKIP | {grp} | Skipping invalid geometries with sci_name: {paste(invalid_sci, collapse = ', ')}")
      a <- a %>% 
        filter(st_is_valid(.))
    }
    ...
    if (length(sci_dupes) > 0)
      log_error("| ERROR | {grp} | duplicate `sci_name` in: {paste(sci_dupes, collapse = ', ')}")
    
    if (any(!st_is_valid(x))){
      log_warn("| WARN | {grp} | repairing invalid geometries after summarizing")
      x <- st_make_valid(x)
    }
  ...
  for (j in 1:nrow(x)){ # j = 1
    ...
    if (inherits(r, "try-error")){
      log_warn("| SKIP | {grp} | error in rasterize() of {j}/{nrow(x)} ({x$sci_name[j]}): {r}")
      next()
    }
    ...
    if (n_notna == 0){
      log_info("| SKIP | {grp} | No layers in rast have notNA after cropping and masking to US EEZ")
      next()
    }
...
```{r update_rng_iucn_to_redlist-value}
...
for (i in 1:length(taxa)){ # 
  ...
  message(glue("Updating taxa:'{taxon}' (mdl_seq:{mdl_seq}; {i}/{length(taxa)}) model_cells to value: {rl_val} ~ {Sys.time()}"))
...
```{r update_rng-iucn-redlist-code_in_taxon}

d_iucn_chg <- tbl(con_sdm, "taxon") |> 
  select(worms_id, scientific_name, common_name, redlist_code_taxon = redlist_code) |> 
  inner_join(
    tbl(con_sdm, "species") |> 
...
d_esa <- tbl(con_sdm, "species") |>
  ...
  select(
    ds_key, 
    worms_id, scientific_name_dataset, common_name_dataset,
    redlist_code, redlist_year) |>
...
dbExecute(
  con_sdm,
  "UPDATE taxon
    SET esa_code = d_esa.ds_key
    FROM d_esa
    WHERE taxon.worms_id = d_esa.worms_id")
```


```{r add_rng_iucn_to_taxon}
...
dbExecute(con_sdm, "ALTER TABLE taxon ADD COLUMN IF NOT EXISTS rng_iucn INTEGER")
...
d_x <- tbl(con_sdm, "taxon") |> 
  filter(
    is.na(mdl_seq)) |>  # consider running: reset mdl_seq for taxa that now need re-merging with IUCN
...
dbExecute(con_sdm,
  "UPDATE taxon
   SET sp_cat = 'turtle'
   WHERE scientific_name IN ('Caretta caretta', 'Lepidochelys olivacea')") # loggerhead, olive ridley
...
for (i in 1:nrow(d_x)){  # i = 1  # which(str_detect(d_x$scientific_name, ".*ricei")) # i = 1512
  ...
  has_iucn_str <- if (has_iucn)
    glue("; Mask applied (because IUCN range map present) from combining datasets: {paste(ds_mask_sp, collapse = ', ')}") else ""
  d_model <- tibble(
    ds_key      = ds_key,
    taxa        = sp_key,
    time_period = "2025",
...
dbExecute(
  con_sdm, 
    ...
    SET is_ok  = d2.is_ok
    FROM d2
    WHERE taxon.taxon_id = d2.taxon_id")  # 17,561
duckdb_unregister(con_sdm, "d2")
```
