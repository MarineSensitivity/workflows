...
d_w <- d_w |>
  ...
  collect() # OLD: 16,947 × 5 -> NEW: 16,947 × 5
...
#    20  40      60   80  100
# 1,189  45      87  157   38
# NEW before alignment with er_score
#   OLD: CR:100/EN:80/VU:60/NT:40/LC:20/DD:20
#   NEW: CR: 50/EN:25/VU: 5/NT: 2/LC: 1/DD:
# NEW BEFORE: (interrupted at i=808)
#    20  40  50  60   80  100
#   537  29 806  60   62   21
...
for (i in 1:length(taxa)) {
  ...
  cell_val <- q_spp |>
    ...
    mutate(
      cell_value = case_match(
        redlist_code,
        # aligned with er_score from taxon_listing_join (v3 scoring)
        "CR" ~ 50L, # critically endangered
        "EN" ~ 25L, # endangered
        "VU" ~ 5L, # vulnerable
        "NT" ~ 2L, # near threatened
        .default = 1L # LC, DD, etc.
      )
    ) |>
    pull(cell_value)

  message(glue(
    "Updating taxa:'{taxon}' (mdl_seq:{mdl_seq}; {i}/{length(taxa)}) model_cells to value: {cell_val} ~ {Sys.time()}"
  ))

  dbExecute(
    con_sdm,
    glue(
      "UPDATE model_cell SET value = {cell_val} WHERE mdl_seq = {mdl_seq}"
    )
  )
}
```
...
# 3 ch_fws   FWS Critical Habitat  TRUE             4
# 4 ch_nmfs  NMFS Critical Habitat TRUE             3
# 5 rng_fws  FWS Range             TRUE             5
# 6 ca_nmfs  NMFS Core Area        TRUE             2
# 7 rng_iucn IUCN Range            TRUE             7
# 8 ms_merge Merged Model          FALSE            0
```
...
for (i in 1:nrow(d_x)) {
  ...
  if (length(mdl_seqs) > 0) {
    dbExecute(
      con_sdm,
      glue("DELETE FROM taxon_model WHERE ds_key = '{ds_key}' AND taxon_id = {d_sp$taxon_id}")
    )
    dbExecute(
      con_sdm,
      glue("DELETE FROM model WHERE ds_key = '{ds_key}' AND taxa = '{sp_key}'")
