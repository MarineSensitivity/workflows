...
```{mermaid}
%%| label: fig-design
%%| fig-cap: "ONE batched merge → TWO surfaces from the same data. GLOBAL viz = am ∪ range (FULL OUTER, whole-range merged model) → COGs. US scoring = v7-faithful US-boundary-aware rule (range∩US max(er,am-at-range) UNION am∩US for taxa with no US range) — NOT a trim of global. Turtles multiplicative."
flowchart LR
  raw["dist/*/*.parquet<br/>(raw model_cell)"] --> mcp[("mc_parts<br/>partitioned by mkey_id (int)")]
  mcp --> gm["GLOBAL: er over range<br/>∪ am over whole am footprint<br/>(FULL OUTER)"]
...
dbExecute(con, glue("CREATE OR REPLACE TABLE turtle_src AS
  SELECT ms_merge_key, ds_key, cell_id, val FROM mc_parts WHERE mkey_id IN ({tids_sql})"))
