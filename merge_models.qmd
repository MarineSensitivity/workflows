...
dbWriteTable(con_sdm, "taxon", d_taxon_clean, append = F)
...
if (dbExistsTable(con_sdm, "listing")) {
  ...
  n_worms <- dbGetQuery(con_sdm,
    "SELECT count(*) AS n FROM taxon WHERE extrisk_code IS NOT NULL")$n
  ...
  n_total <- dbGetQuery(con_sdm,
    "SELECT count(*) AS n FROM taxon WHERE extrisk_code IS NOT NULL")$n
  message(glue("Listing joined by botw_id: {n_total - n_worms} additional taxa"))
...
metadata <- tribble(
  ...
  "rng_iucn", "IUCN Range",              "CR:100, EN:80, VU:60, NT:40, LC:20, DD:20",  TRUE,     7L,
