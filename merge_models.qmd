...
# SPATIAL extinction risk (per-cell ER x suitability, msens::turtle_sql): the sea turtles since
# v4b, and from v9.1 every species with an NMFS listed entity below species level (dps_nmfs:
# humpback, Southern Resident killer whale, salmonid ESUs, ...). For these the plain rule's
# `max(er, suit)` at a species-wide ER is exactly what painted the humpback a flat 100.
er_ds     <- c(turtle_ds, "dps_nmfs")
...
er_like     <- paste(sprintf("mdl_key LIKE '%s|%%'", er_ds), collapse = " OR ")
turtle_keys <- dbGetQuery(con, glue(
  "SELECT DISTINCT ms_merge_key FROM taxon_model WHERE {er_like}"))$ms_merge_key
dps_keys    <- dbGetQuery(con, "SELECT DISTINCT ms_merge_key FROM taxon_model WHERE mdl_key LIKE 'dps_nmfs|%'")$ms_merge_key
...
log_info("{length(turtle_keys)} spatial-ER taxa (multiplicative merge): {length(turtle_keys) - length(dps_keys)} turtles + {length(dps_keys)} NMFS-DPS species")
...
# the critical-habitat datasets are NOT an override for dps_nmfs taxa: dps_nmfs was built from the
# same designations WITH each entity's status, while ch_* carries the species-level ER (100 for the
# humpback) and would paint the Threatened Mexico-DPS habitat back to Endangered.
dps_sql <- if (length(dps_keys)) paste(sprintf("'%s'", dps_keys), collapse = ",") else "''"
dbExecute(con, glue("CREATE OR REPLACE TABLE turtle_src AS
  SELECT ms_merge_key, ds_key, cell_id, val FROM {parts_sql(turtle_ids)}
  WHERE NOT (ds_key IN ({ch_sql}) AND ms_merge_key IN ({dps_sql}))"))
dbExecute(con, paste("CREATE OR REPLACE TABLE mc_turtle AS",
  msens::turtle_sql(er_ds, suit_ds, ch_ds, src = "turtle_src")))
...
Turtles are excluded: their footprint is the DPS extinction-risk surface, not the union of every
non-am dataset, so the identity doesn't apply — `msens::turtle_sql()` masks them by construction and
`test-merge.R` asserts it.
