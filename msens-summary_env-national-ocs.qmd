...
librarian::shelf(
  bslib, DBI, dplyr, duckdb, DT, fs, ggiraph, ggplot2, glue, here, htmlwidgets,
  kableExtra, knitr, mapview, purrr, RColorBrewer, readr, sf, shiny, stringr, 
  terra, tibble, tidyr, tidyverse, webshot2,
  quiet = T)
...
for (is_ak in c(T, F)){
  ...
  b       <- here(glue("figs/msens-summary_env-national-ocs/map_pa_{ifelse(is_ak, 'ak','l48')}"))
  b_html  <- glue("{b}.html")
...
er_fl_png <- here("figs/msens-summary_env-national-ocs/ecoregion_flower-plot_scores.png")
er_fl_pdf <- here("figs/msens-summary_env-national-ocs/ecoregion_flower-plot_scores.pdf")
...
```{r primprod_pa_avg_stddev}
# contributing metrics
metric_input_keys <- glue("primprod_{c('avg','stddev')}")
m_input_seqs <- tbl(con_sdm, "metric") |> 
  collect() |> 
  View()
  filter(metric_key %in% metric_input_keys) |> 
...
ggplot(d_pp, aes(x=planarea_key, y=primprod_avg)) +
  geom_bar(
    position=position_dodge(), stat="identity", color='darkgreen') +
...
d_pp |> 
  mutate(
    primprod_avg <- primprod_avg |> set_units("g m^ km^-2 yr^-1"),
    primprod_avg <- primprod_avg * set_units(1, "t C km^-2 yr^-1"),
  )
