...
er_pa_n_csv    <- glue("{dir_data}/derived/tbl_er_pa_scores.csv")
pa_n_csv       <- glue("{dir_data}/derived/tbl_pa_scores.csv")
...
plot_flower <- function(
    ...
    fld_height,
    fld_width,
    colors,      # required for scale_fill_manual
    tooltip_expr = NULL,
    score        = NULL,
    title        = NULL,
    interactive  = T){
...
for (is_vgpm in c(T, F)){  # is_vgpm = T
...
d_er_pa_n |> 
  ...
  # OLD: row_lbl, region_key, region_name, ecoregion_key, ecoregion_name, row_type, planarea_key, planarea_name, pct_planarea_in_ecoregion, area_km2, score, primprod, sea_turtle, other, mollusk, marine_mammal, fish, crustacean, pct
  # NEW: row_lbl, region_key, region_name, ecoregion_key, ecoregion_name, row_type, planarea_key, planarea_name, pct_planarea_in_ecoregion, area_km2, score, primprod, reptile, other, mammal, invertebrate, fish, coral, bird, all, pct
  # select(-region_key, -pct) |> 
  select(
    row_type, 
    region_name, 
    ecoregion_name, ecoregion_key, 
    planarea_name, planarea_key, pct_planarea_in_ecoregion, 
    row_lbl,
    score,
    primprod, 
    # crustacean, fish, marine_mammal, mollusk, sea_turtle, other,
    bird, coral, fish, invertebrate, mammal, reptile, other,
    area_km2) |>
  # View()
  write_excel_csv(er_pa_n_csv)
...
gt_er_pa_n <- d_er_pa_n |> 
  select(
    ...
    primprod, bird, coral, fish, invertebrate, mammal, reptile, other, 
    area_km2) |> 
  gt() |> 
  sub_missing(
  columns = everything(),
  ...
  fmt_number(
    columns  = c(
      score, primprod, bird, coral, fish, invertebrate, mammal, reptile, other),
    decimals = 0) |>
  fmt_number(
    columns  = c(area_km2),
...
tbl_docx <- glue("{dir_figs}/table_planarea_scores.docx")
...
d_pa_n <- d_er_pa_n |> 
  filter(row_type == "planarea") |> 
  arrange(desc(score)) |> 
  select(
    -row_type, 
    -ecoregion_name, -ecoregion_key, 
    -pct_planarea_in_ecoregion) |> 
  mutate(
    area_kkm2 = area_km2 / 1000) |>  # convert to 1,000 km²
  mutate(across(where(is.numeric), round)) |>
  mutate(
    row_lbl = glue("{planarea_name} ({planarea_key})") ) |>
  distinct(
    row_lbl, score, primprod, 
    bird, coral, fish, invertebrate, mammal, reptile, other, 
    area_kkm2) |> 
  select(
    `Planning Area`      = row_lbl, # row_type,
    Score                = score, 
    ...
    `Area (1,000 km^2^)` = area_kkm2)
# View(d_pa_n)
write_excel_csv(d_pa_n, pa_n_csv, na = "")
...
# OLD: "crustacean","fish","marine mammal","mollusk","other","primprod","sea turtle"
components <- c(
...
```{r}
#| label: plot_planarea_flowers
# pa_l48ak ----
source(here("libs/db.R")) # con

pa_l48ak <- tbl(con, "ply_planareas_2025") |> 
  filter(
    region_key == "AK" | 
...
d_scores <- tbl(con_sdm, "zone") |> 
  filter(
    fld == "planarea_key",
    value %in% pa_l48ak$planarea_key) |>   # 36 -> 27
  select(planarea_key = value, zone_seq) |>
  inner_join(
    tbl(con_sdm, "zone_metric") |> 
      filter(metric_seq == !!m_key_score),
    by = join_by(zone_seq)) |> 
  select(planarea_key, score = value) |> 
  mutate(score = round(score)) |> 
  collect() |> 
  arrange(desc(score), planarea_key)

d_scores$planarea_lbl <- factor(
  d_scores$planarea_key,
  levels = d_scores$planarea_key, ordered = T)
  
# d_fl ----
d_fl <- tbl(con_sdm, "zone") |> 
  filter(
    fld == "planarea_key",
    value %in% pa_l48ak$planarea_key) |> 
  select(planarea_key = value, zone_seq) |>
  inner_join(
    tbl(con_sdm, "zone_metric"),
    by = "zone_seq") |>
  ...
  mutate(
    component = metric_key |>
      ...
      str_replace("_ecoregion_rescaled","") |>
      str_replace("_", " ")) |> 
    # TODO: after expand_grid # even = 1) |> 
  select(planarea_key, component, value) |> 
  arrange(planarea_key, component, value)  # 211 × 3

# add missing components with value 0
#   to prevent erroneous color matching
# TODO: try graying out since NA (vs 0)
d_0 <- expand_grid(
  planarea_key = d_scores$planarea_key,
  component    = sort(unique(d_fl$component)),
  value        = 0) |>  # 216 x 3
  anti_join(
    d_fl,
    by = join_by(planarea_key, component)) # 5 x 3

d_fl <- d_fl |> 
  bind_rows(d_0) |> 
  arrange(planarea_key, component) |> 
  mutate(
    even = 1)
...
d_fl <- d_fl |> 
  ...
  arrange(planarea_lbl, component)
    
# rphylopic ----
# - [Acropora palmata by Victor Piñon-González (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/32eeaa29-2e90-40b5-92fb-eb8d7fcab9ab/acropora-palmata)
# - [Laridae by Sean McCann (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/6f87dbf2-289a-4c57-b26c-9384993c37d4/laridae)
# - [Epinephelus flavocaeruleus by Margot Michaud (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/d1ddd347-b0c4-4ab2-8315-978a5bfecb84/epinephelus-flavocaeruleus)
# - [Octopus bimaculatus (PDM 1.0) - PhyloPic](https://www.phylopic.org/images/d115b40d-efe3-487e-bed1-4f6aace5c814/octopus-bimaculatus)
# - [Tursiops truncatus (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/388e792c-f8fd-4bd8-ad38-e251b5244ac0/tursiops-truncatus)
# - [Capitella by B. Duygu Özpolat (CC BY-NC-SA 3.0) - PhyloPic](https://www.phylopic.org/images/5b80cf50-11f0-4e9a-b23b-452fac47615b/capitella)
# - [Ceratium hirundinella (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/573efc92-2521-453c-9fa3-0b356d78b910/ceratium-hirundinella)
# - [Caretta caretta by James R. Spotila and Ray Chatterji (CC0 1.0) - PhyloPic](https://www.phylopic.org/images/b3e35f7c-af3b-44dc-9680-82bd2d6392fb/caretta-caretta)

# one flower ----
# pa_key <- "FLS"
pa_key <- "WGA"
# svg <- get_phylopic("b3e35f7c-af3b-44dc-9680-82bd2d6392fb")

pa_name <- read_csv(er_pa_csv) |> # names() |> paste(collapse = ", ") |> cat()
  distinct(
...
g <- d_fl |>
  ...
  plot_flower(
    ...
    fld_height   = value,
    fld_width    = even,
    tooltip_expr = "{component}: {round(value, 2)}",
    # title        = pa_key,
    title        = pa_name,
    score        = d_scores |> 
      filter(planarea_key == !!pa_key) |> 
      pull(score),
    colors       = component_cols,
    interactive  = F) + 
  theme_minimal() +
  # theme_void() +
  theme(
   ...
   #     x = img = svg), 
   #   height = 0.1)
fl_png <- glue("{dir_figs}/planarea_flower-plot_scores_{pa_key}.png")
fl_pdf <- glue("{dir_figs}/planarea_flower-plot_scores_{pa_key}.pdf")
png(fl_png, width = 1200, height = 800, res = 150)
print(g)
dev.off()
pdf(fl_pdf, width = 7, height = 5)
print(g)
dev.off()
...
for (theme_name in c("minimal","linedraw","min_line","dark","void")) {
  ...
  g <- d |> 
    ...
    scale_fill_manual(
      ...
      name   = "Component") +
    facet_wrap(~planarea_lbl) +
    coord_polar(theta = "y") +
    # create donut hole
    xlim(c(-20, max(d_scores$score, d$xmax))) +
    # add white circle to fill donut hole
    annotate(
    ...
    # apply chosen theme with theme_fxn()
    theme_fxn() +
    theme(
      legend.position = "bottom",
      legend.title    = element_text(size = 8),
  ...
  pa_fl_png <- glue("{dir_figs}/planarea_flower-plot_scores_{theme_name}.png")
  pa_fl_pdf <- glue("{dir_figs}/planarea_flower-plot_scores_{theme_name}.pdf")
