...
ply <- read_sf(gdb, layer = "All_critical_habitat_poly_20230724", quiet = T) |> 
  ...
  mutate(
    ...
    esa_code = case_match( # er: extinction risk
      liststatus,
      "Threatened" ~ "TN",     # "TN" = 0.7
      "Endangered" ~ "EN") |>  # "EN" = 0.9
...
## Species Directory - All Species

- [Species Directory | NOAA Fisheries](https://www.fisheries.noaa.gov/species-directory)
- [Glossary: Classifications for Protected Species | NOAA Fisheries](https://www.fisheries.noaa.gov/national/glossary-classifications-protected-species)

```{r}
#| label: scrape_species-directory_all

librarian::shelf(
  dplyr, here, purrr, rvest, stringr, tibble,
  quiet = T)
...
# crawl with pagination (standard HTTP, no chromote) ----
crawl_noaa_species <- function(verbose = TRUE) {

  if (verbose) {
    message("NOAA Fisheries Species Directory Crawler")
    message("Method: Standard HTTP (read_html) with pagination")
  }
...
# run crawler and save ----
d_spp <- crawl_noaa_species(verbose = TRUE)
write.csv(d_spp, spp_csv, row.names = FALSE)
message(sprintf("Saved to: %s", spp_csv))
...
```{r}
#| label: scrape_species-directory_esa-threatened-endangered
