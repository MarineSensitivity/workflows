...
dir_figs <- here(glue("figs/msens-summary_programareas_{v_sfx}"))
dir_dl <- glue("{dir_derived}/downloads")
mapbox_tkn_txt <- glue("{dir_private}/mapbox_token_bdbest.txt")

# input files (versioned) ----
sdm_dd <- glue("{dir_derived}/sdm{v_sfx}.duckdb")
...
# mask to program areas (l48) + ecoregions (ak) to exclude Atlantic coast
...
er_tbl <- glue("ply_ecoregions_2025{v_sfx}")
d_fl <- tbl(con_sdm, "zone") |>
  ...
  collect() |>
  left_join(
    tbl(con, glue("ply_ecoregions_2025{v_sfx}")) |>
      filter(
        region_key == "AK" |
...
message(glue(
  "Copied {length(dl_data)} data files and {length(fig_files)} figure files to downloads/"
))
```
