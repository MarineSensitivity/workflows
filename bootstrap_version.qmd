...
librarian::shelf(DBI, dplyr, duckdb, fs, glue, here, jsonlite, logger, purrr, readr, tibble, quiet = TRUE)
...
# ingest target -> ds_key, from each ingest notebook's msens: block (the same parser the DAG uses)
wf <- msens::parse_qmd_frontmatter(dir_ls(here(), glob = "*.qmd"))
ing <- wf |> filter(workflow_type == "ingest") |>
  mutate(ds_key = map_chr(dataset, \(d) d$ds_key %||% NA_character_)) |>
  filter(!is.na(ds_key)) |> select(target_name, ds_key, output)
