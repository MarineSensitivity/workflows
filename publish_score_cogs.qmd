...
# parameterized by version so the SAME notebook publishes score COGs for a
# backfilled release; defaults to the configured `ver`
sver    <- params$ver %||% ver
grid_id <- msens::grid_for_ver(sver)
grid    <- msens::grid_spec_for(grid_id)
db      <- path.expand(msens::sdm_db_path(sver))
dir_v   <- dirname(db)
dir_cog <- file.path(dir_v, "score_cogs")
...
# v8 renamed the value column to `val` (`value` is a DuckDB reserved word)
cmv <- if ("val" %in% dbListFields(con, "cell_metric")) "val" else "value"
zv  <- if ("val" %in% dbListFields(con, "zone"))        "val" else "value"
log_info("ver={sver} grid={grid_id} ({grid$nc}x{grid$nr}) cell_metric.{cmv} s3={do_s3}")
```
...
score_cog <- h |>
  ...
  ungroup() |>
  transmute(ver = sver, grid_id, metric_key, subregion_key, payload_hash, content_hash,
            cog_url = url, rescale_min, rescale_max, colormap = "spectral_r")

f_reg <- glue("{dir_v}/tables/score_cog.parquet")
