...
stopifnot("AquaX modeled land cells (not in the ocean cell table)" = cov$n_land == 0)
msens::report_table(cov, caption = "AquaX mask vs the cell grid: every modeled cell is ocean; the uncovered in_usa cells are where AquaMaps continues to apply")
```
...
Ranked by |Δ mean suitability| on shared cells, among taxa sharing ≥ 100 cells (so a handful of
edge cells cannot top the list). Each name opens the species on the **preview host**, where the
...
ranked <- all_ok |> filter(n_shared >= 100) |> left_join(reg_w, by = "taxon_id") |>
