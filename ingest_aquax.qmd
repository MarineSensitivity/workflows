...
3. **COGs are built here** (`AX_COG=1`, uploaded with `AX_COG_S3=1`): the delivered band
   bit-exact as the *native* representation (`native/ax_native/`, Float32 0–1000, AUC/TSS/cutoff as
   metadata) and the ingested surface as the *model* representation (`native/ax/`, 1–100) — the same
   two representations AquaMaps has, so the species app's Original/Interpolated toggle needs no
   change (D6/D7). `publish_native.qmd` registers them from `model_ax.csv`; it never repaints.
