...
Method (the proven v7 `resample_smooth` fade): take the native 0.5° raster
(`get_species_raster()`, 1-100, absent = NA, from `am.duckdb` or the cached tif),
**extend it by a 1° margin and zero-fill** (so absent cells surround the range),
**bilinear-resample onto the 0.05° grid**, mask to ocean, drop values `< threshold`.
`cell_id` is the global raster index (`terra::cellFromXY` on Bio-Oracle) — the same
`cell_id` as the `cell` table. No projection (Bio-Oracle/AquaMaps/AquaX share the
0.05° topology). Global (no US clip).
...
margin    <- 1                                                        # deg zero-surround margin
...
interp_sp <- function(sp_key) {
  ...
  # zero-surround: extend by margin, fill NA with 0, bilinear -> 0.05°, mask ocean
  bb     <- terra::ext(r_sp) + margin
  r_sp_z <- terra::extend(r_sp, bb); r_sp_z[is.na(r_sp_z)] <- 0
  tmpl   <- terra::crop(r_bo, bb)
