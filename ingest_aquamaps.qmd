...
if (redo_am_w05 || !file_exists(w05_db)) {
  ...
  oc <- as.data.frame(rast(cellid_tif), xy = TRUE, na.rm = TRUE)   # ocean cells: lon, lat, cell_id
...
for (i in seq_along(todo)) {
  ...
  dbExecute(con, glue("
    COPY (
      SELECT 'am|{k}' AS mdl_key, w.cell_id, round(SUM(w.w * sc.prob), 2) AS \"value\"
      FROM w05 w
