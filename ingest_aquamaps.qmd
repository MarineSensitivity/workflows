...
interp_sp <- function(sp_key) {
  ...
  r_bo <- terra::rast(bio_oracle_tif, lyrs = "depth_mean")       # 0.05° ocean template + cell geometry

  # v7-identical fade: zero-fill absent cells WITHIN the (trimmed) range extent,
  # bilinear-resample onto the 0.05° grid, mask to ocean. No outward margin —
  # bilinear does not extrapolate past the source extent, so this reproduces the
  # v7 iterate/resample_smooth result (verified ~identical, cor > 0.999).
  r_sp_z <- r_sp; r_sp_z[is.na(r_sp_z)] <- 0
  tmpl   <- terra::crop(r_bo, terra::ext(r_sp))
  r05    <- terra::mask(terra::resample(r_sp_z, tmpl, method = "bilinear"), tmpl)

  d <- terra::as.data.frame(r05, xy = TRUE, na.rm = TRUE)
  names(d) <- c("lon", "lat", "value")
  d <- d[d$value >= threshold, , drop = FALSE]
  if (nrow(d) == 0) return("empty")

  arrow::write_parquet(tibble(
    mdl_key = glue("{ds_key}|{sp_key}"),
    cell_id = as.integer(terra::cellFromXY(r_bo, cbind(d$lon, d$lat))),
