...
**Parallelized with `furrr`** — one worker per species; each opens its own
read-only `am.duckdb` connection and reads rasters from files (terra objects are
not shared across workers), and writes its own Parquet file (no shared writes). The
run is **resumable** — a species whose Parquet exists is skipped — so it can run
over hours/days on the server and restart after interruption.

```{r}
#| label: v8_cell_ingest
#| eval: false

librarian::shelf(arrow, DBI, dplyr, duckdb, fs, furrr, future, glue, logger,
                 purrr, readr, terra, tibble, quiet = T)
...
# run in parallel, resumable
plan(multisession, workers = n_workers)
log_info("interpolating {format(nrow(sp_all), big.mark=',')} AquaMaps species on {n_workers} workers ...")
res <- future_map_chr(sp_all$sp_key, purrr::possibly(interp_sp, otherwise = "error"),
