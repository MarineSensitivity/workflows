...
species_contributions <- dbGetQuery(
  ...
  SELECT 
    m.taxa,
    mc.value as suitability
  FROM model_cell mc
  JOIN model m ON mc.mdl_seq = m.mdl_seq
  WHERE mc.cell_id = {cell_id} AND mc.value > 0"
  )
) |>
...
# get primary productivity value for this cell
...
species_zone_contributions <- dbGetQuery(
  ...
  SELECT 
    m.taxa,
    SUM(mc.value * zc.pct_covered) / SUM(zc.pct_covered) as avg_suitability,
...
# calculate percent contribution of each component to the final score
score_value <- zone_metrics |> filter(metric_key == m_key) |> pull(value)
