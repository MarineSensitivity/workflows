...
cell ||--o{ zone_cell : cell_id
metric ||--o{ cell_metric : metric_seq
metric ||--o{ zone_metric : metric_seq
region ||--o{ zone_metric : rgn_seq
region ||--o{ zone_cell : rgn_seq
```
...
if (
  dbExistsTable(con_sdm, "model_cell") || dbExistsTable(con_sdm, "sdm_values")
) {
  tables_to_drop <- c(
    "zone_metric",
    "zone_cell",
...
# dbExecute(con_sdm, "DROP TABLE IF EXISTS region")
...
# dbExecute(con_sdm, "DROP TABLE IF EXISTS region_metric")
dbExecute(
  con_sdm,
  "
...
# dbExecute(con_sdm, "DROP TABLE zone_cell")
...
```{r}
#| label: insert_dataset_row

# add AquaMaps as a dataset
...
```{r}
#| label: skip_load_planning_areas
...
if (!dbExistsTable(con_sdm, "ply_planareas")) {
  ...
  dbExecute(
    con_sdm,
    "CREATE INDEX idx_ply_planareas_geom ON ply_planareas USING RTREE (geom)"
  ) # TODO: redo with USING RTREE
}
...
dbExecute(
  con_sdm,
  "
  CREATE INDEX idx_cell_geom ON cell USING RTREE (geom)"
) # TODO: redo with USING RTREE
...
# dbExecute(con_sdm, "DROP TABLE IF EXISTS region_cell")
# dbExecute(con_sdm, "
...
cell_tif <- glue("{dir_data}/derived/r_bio-oracle_planarea.tif")
spp_pa_csv <- glue(
  "{dir_data}/derived/aquamaps.org/spp_boem-planareas-2025.csv"
)
...
for (i in 1:n) {
  ...
  if (length(mdl_seqs > 0)) {
    dbExecute(
      con_sdm,
      glue("DELETE FROM model WHERE ds_key = 'am_0.05' AND taxa = '{sp_key}'")
    )
    dbExecute(
      con_sdm,
      glue(
    ...
    dbExecute(
      con_sdm,
      glue("DELETE FROM species WHERE ds_key = 'am_0.05' AND taxa = '{sp_key}'")
    )
  }
  ...
  d_species <- tibble(
    ...
    scientific_name = glue("{sp_info$genus[1]} {sp_info$species[1]}"), # TODO: scientific_name_data…¹ common_name_dataset
  ...
  d_mdl_cell <- as.data.frame(r_ds, cells = T, na.rm = T) |>
    ...
    select(cell_id = cell, mdl_seq, val) |>
    arrange(cell_id)
...
- [ ] ∆ sp_cat: marine mammal -> mammal; + coral; + invertebrate; + bird
- [ ] combine 
- [ ] include species without IUCN or ESA as LC 0.2
- [ ] + TN "Threatened" 0.6
- [ ] ensure both sp_wt and sp_suit are multiplied within same range, ie [0,1] or [0,100]

```{r}
#| label: chg_sp_cat
# TODO: above

# 2025-07-24: after adding am_0.05 and bl bird datasets, before ch_nmfs & ch_fws datasets
d_species <- tbl(con_sdm, "species") |>
  collect() |>
  rename(sp_cat_old = sp_cat) |>
  left_join(
    tbl(con_am, "spp") |>
      select(
        sp_key,
    ...
    sp_cat = case_when(
      ...
      ds_key == "am_0.05" &
        class %in%
          c(
            "Elasmobranchii"
          ) ~ "shark", # TODO: add shark (explain, rays, skates)
      ds_key == "am_0.05" &
        class == "Mammalia" ~ "mammal",
      ds_key == "am_0.05" &
        class == "Reptilia" ~ "turtle",
      ds_key == "am_0.05" &
        class == "Aves" ~ "bird",
      ds_key == "am_0.05" &
        class == "Anthozoa" ~ "coral",
      ds_key == "am_0.05" &
        m_invertebrates == 1 ~ "invertebrate",
      .default = sp_cat_old
    )
  )

# d_species |> select(sp_cat_old, sp_cat) |> table()
#                sp_cat
# sp_cat_old      bird coral fish invertebrate mammal other turtle
#   bird           504     0    0            0      0     0      0
#   crustacean       0     0    0         3025      0     0      0
#   fish             0     0 6328            0      0     0      0
#   marine mammal    0     0    0            0     85     0      0
#   mollusk          0     0    0         3920      0     0      0
#   other            1   807  354         1580      0  1417      0
#   sea turtle       0     0    0            0      0     0     33

d_species_new <- d_species |>
  select(-sp_cat_old, -class, -m_invertebrates)
dbWriteTable(con_sdm, "species", d_species_new, overwrite = T)

# update sp_seq to be an integer sequence
dbExecute(con_sdm, "ALTER TABLE species ALTER COLUMN sp_seq TYPE INTEGER;")
dbExecute(
  con_sdm,
  "ALTER TABLE species ALTER COLUMN sp_seq SET default nextval('species_sp_seq_seq');"
)
...
#  505          807         6682         8525           85         1417           33

# TODO: add Macrocystis pyrifera
# tbl(con_sdm, "species") |>
#   # collect() |>
#   filter(str_detect(scientific_name_accepted, "Macrocystis"))

d_spp_am_other <- tbl(con_am, "spp") |>
  collect() |>
  inner_join(
    tbl(con_sdm, "species") |>
      select(sp_key, sp_cat) |>
      collect(),
    by = "sp_key"
  ) |>
  filter(sp_cat == "other") |>
...
#    class                n
#    <chr>            <int>
#  1 Polychaeta         575 # worms
#  2 Ascidiacea         257 # sea squirts
#  3 Gymnolaemata        98 # class of Bryozoans, sessile, grow on surfaces of rocks, kelp, and in some cases on animals, like fish.
#  4 Florideophyceae     94 # red algae
# TODO: Kingdom: Plantae and other habitat forming species (kelp, mangroves, seagrasses)
#  5 Not assigned        61
#  6 Bryopsidophyceae    58
#  7 Polythalamea        45
#  8 Phaeophyceae        41
#  9 Sipunculidea        32
# 10 Ulvophyceae         30
...
richness_metric_seq <- metric_ids$metric_seq[
  metric_ids$metric_key == "species_richness"
]
...
system.time({
  ...
  SELECT 
    ...
    COUNT(DISTINCT m.taxa) as val
  FROM model_cell mc
  JOIN model m ON mc.mdl_seq = m.mdl_seq
  WHERE mc.val > 0.5  -- presence threshold
  GROUP BY mc.cell_id"
    )
  )
}) # 34.346 sec
...
shannon_metric_seq <- metric_ids$metric_seq[
  metric_ids$metric_key == "shannon_index"
]
...
FROM (
  SELECT 
    mc.cell_id,
    mc.val / SUM(mc.val) OVER (PARTITION BY mc.cell_id) as p
  FROM model_cell mc
  WHERE mc.val > 0
) t
WHERE p > 0
...
register_zones <- function(ply, tbl, fld) {
  ...
  dbExecute(
    con_sdm,
    glue("DELETE FROM zone WHERE tbl = '{tbl}' AND fld = '{fld}'")
  )
...
insert_zone_cell_data <- function(df, tbl, fld) {
  ...
  d_zone <- dbGetQuery(
    ...
    glue(
      "
    SELECT zone_seq, val FROM zone WHERE tbl = '{tbl}' AND fld = '{fld}'"
    )
  )
  ...
  dbExecute(
    con_sdm,
    glue(
      "DELETE FROM zone_cell WHERE zone_seq IN ({paste(d_zone$zone_seq, collapse = ',')})"
    )
  )
...
for (sp_cat in sp_cats) {
  ...
  for (fxn in c("min", "max")) {
    ...
    dbExecute(
      ...
      SELECT 
        z.zone_seq,
        -- z.tbl, z.fld, z.val AS tbl_fld_val,
        {m_seq} as metric_seq,
        {fxn}(cm.val) as val
      FROM zone z
      ...
      -- GROUP BY z.zone_seq, z.tbl, z.fld, z.val
      GROUP BY z.zone_seq "
      )
    )
  }
  ...
  dbExecute(
    ...
    glue(
      ...
      ecoregion_minmax AS (
        SELECT 
          z.zone_seq,
          MIN(zm.val) as min_value,
          MAX(zm.val) as max_value
        FROM zone z
...
## OLD: Transfer raw PrimProd into cell_metric

```{r}
#| label: old_primprod_cell_metric
#| eval: false

# Create new metric for rescaled primary productivity
m_key <- "primprod"
m_description <- "Primary productivity: Average Total Surface Phytoplankton 2010-2020 (mmol/m^3; Bio-Oracle dataset ID: phyc_baseline_2000_2020_depthsurf)"
...
dbExecute(
  ...
  glue(
    ...
    SELECT 
      cell_id,
      {m_seq} as metric_seq,
      prim_prod_mean AS val
    FROM cell c
    WHERE 
    prim_prod_mean IS NOT NULL
"
  )
)
```

## NEW: Transfer raw PrimProd into cell_metric
...
m_key <- "primprod"
m_description <- "Primary productivity: Oregon State Vertically Generalized Production Model (VGPM) from Visible Infrared Imaging Radiometer Suite (VIIRS) satellite data (mg C / m^2 / day)"
# TODO: consider log rescaling PrimaryProductivity
...
#   summary()
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max.
# 1533    7708   12593   21528   30982  282413

# d_metric <- tbl(con_sdm, "metric") |> collect()
# View(d_metric)
#
# d_ecoregion_minmax <- dbGetQuery(con_sdm, "
#   SELECT
#     z.zone_seq,
#     MIN(zm.val) as min_value,
#     MAX(zm.val) as max_value
...
for (m_key in metrics) {
  ...
  dbExecute(
    ...
    SELECT 
      zc.zone_seq,
      {m_seq} as metric_seq,
      SUM(cm.val * zc.pct_covered) / SUM(zc.pct_covered) as val
    FROM zone_cell zc
...
```{r}
#| label: new_calc_planarea_score_from_planarea_metrics
...
```{r}
#| label: new_calc_ecoregion_score_from_ecoregion_metrics
...
```{r}
#| label: pg_update_planarea_ecoregion_metrics
source(here("libs/db.R")) # con
# dbListTables(con)
...
d_pa_metrics <- tbl(con_sdm, "zone") |>
  ...
  select(planarea_key = val, zone_seq) |>
  left_join(
    tbl(con_sdm, "zone_metric") |>
      mutate(
        value = round(val, 1)
  ...
  select(planarea_key, metric_key, val) |>
  filter(
    !is.na(planarea_key),
    !is.na(metric_key),
    !is.na(val)
...
d_er_metrics <- tbl(con_sdm, "zone") |>
  ...
  select(ecoregion_key = val, zone_seq) |>
  left_join(
    tbl(con_sdm, "zone_metric") |>
      mutate(
        value = round(val, 1)
  ...
  select(ecoregion_key, metric_key, val) |>
  filter(
    !is.na(ecoregion_key),
    !is.na(metric_key),
    !is.na(val)
...
## OLD: Calculate Score per PlanArea from cell_metric

```{r}
#| label: old_calc_planarea_score_from_cell_metrics
# calculate score per planning area
m_key <- "score_extriskspcat_primprod_ecoregionrescaled_equalweights"

# get metric_seq for the score metric
m_seq <- tbl(con_sdm, "metric") |>
  filter(metric_key == !!m_key) |>
  pull(metric_seq)
...
results <- dbGetQuery(
  ...
  glue(
    "
  SELECT 
    z.val as planarea_key,
    zm.val as score
  FROM zone_metric zm
  JOIN zone z ON zm.zone_seq = z.zone_seq
  WHERE 
    zm.metric_seq = {m_seq}
    AND z.tbl = 'ply_planareas_2025'
  ORDER BY zm.val DESC
  LIMIT 10"
  )
)
...
## Table of Species Contributions to Score per Cell

```{r}
#| label: species_contributions_to_cell_table
# calculate species contributions to cell score
m_key <- "score_extriskspcat_primprod_ecoregionrescaled_equalweights"
cell_id <- 1772473 # example cell

# get all component metrics that make up the score
sp_cats <- tbl(con_sdm, "species") |> distinct(sp_cat) |> pull(sp_cat)
metric_keys <- c(
  sapply(
    sp_cats,
...
# get redlist codes and scores for species
...
# get species-level contributions to extinction risk for this cell
# first, get the ecoregion rescaling factor for this cell
# TODO: include ecoregion_rescale later -- too complicated for now
# ecoregion_rescale <- dbGetQuery(con_sdm, glue("
#   WITH
#   -- get ecoregion min/max for all species categories
#   ecoregion_minmax AS (
#     SELECT
#       z.zone_seq,
#       m.metric_key,
...
#     FROM zone z
#     JOIN zone_metric zm USING (zone_seq)
#     JOIN metric m ON zm.metric_seq = m.metric_seq
#     WHERE
#       z.fld = 'ecoregion_key' AND
#       m.metric_key LIKE '%_ecoregion_min' OR m.metric_key LIKE '%_ecoregion_max'
#     GROUP BY z.zone_seq, m.metric_key
#   ),
#   -- get cell ecoregion coverage
#   cell_ecoregion AS (
#     SELECT
#       zc.cell_id,
#       z.zone_seq,
#       z.val as ecoregion_key,
#       zc.pct_covered
#     FROM zone_cell zc
#     JOIN zone z ON zc.zone_seq = z.zone_seq
#     WHERE z.fld = 'ecoregion_key' AND zc.cell_id = {cell_id}
#   )
#   SELECT * FROM cell_ecoregion"))

# get raw species contributions (suitability * rl_score)
species_contributions <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT 
    m.taxa,
    mc.val as suitability
  FROM model_cell mc
  JOIN model m ON mc.mdl_seq = m.mdl_seq
  WHERE mc.cell_id = {cell_id} AND mc.val > 0"
  )
) |>
  left_join(d_rl, by = "taxa") |>
  mutate(
    contribution = suitability * rl_score / 100
  ) |> # suitability is 0-100
  tibble() |>
  relocate(contribution)
# species_contributions

# calculate total extinction risk per species category
sp_cat_totals <- species_contributions |>
  group_by(sp_cat) |>
  summarize(
    extrisk_raw = sum(contribution, na.rm = TRUE),
    n_species = n(),
    .groups = "drop"
  )

# get primary productivity val for this cell
prim_prod <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT prim_prod_mean 
  FROM cell 
  WHERE cell_id = {cell_id}"
  )
)$prim_prod_mean

# get the actual metric values for this cell
cell_metrics <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT 
  ...
  WHERE 
    cm.cell_id = {cell_id}
    AND m.metric_key IN ({paste(shQuote(c(metric_keys, m_key)), collapse=', ')})"
  )
)

# calculate percent contribution of each component to the final score
score_value <- cell_metrics |> filter(metric_key == m_key) |> pull(val)
component_values <- cell_metrics |> filter(metric_key != m_key)
component_contributions <- component_values |>
  mutate(
    percent_contribution = round(
      val / nrow(component_values) / score_value * 100,
      1
    )
  )

# create detailed species table
species_table <- species_contributions |>
  # filter(!is.na(rl_score)) |>
  mutate(
    pct_total_extrisk = round(
      contribution / sum(contribution, na.rm = T) * 100,
      2
    ),
    is_pct_na = is.na(pct_total_extrisk)
  ) |>
  arrange(is_pct_na, desc(pct_total_extrisk), scientific_name_dataset) |>
  select(
    is_pct_na,
    pct_total_extrisk,
    taxa,
...
table(species_table$is_pct_na)
table(is.na(species_table$contribution))

# pct_total_extrisk) # |>
# head(20)  # top 20 species

print("Component contributions to cell score:")
print(component_contributions)

print("\nTop species contributions to extinction risk:")
print(species_table)
```

## Table of Species Contributions to Score per PlanArea

```{r}
#| label: species_contributions_to_zone_table
# calculate species contributions to zone (planning area) score
m_key <- "score_extriskspcat_primprod_ecoregionrescaled_equalweights"
planarea_key <- "ALA" # example planning area

# get zone_seq for the planning area
...
# get all cells in this planning area with their coverage percentages
cells_in_zone <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT 
    ...
    pct_covered
  FROM zone_cell
  WHERE zone_seq = {z_seq}"
  )
)

# get species categories and metric keys
sp_cats <- tbl(con_sdm, "species") |> distinct(sp_cat) |> pull(sp_cat)
metric_keys <- c(
  sapply(
    sp_cats,
...
# get species contributions across all cells in the zone
# weighted by cell coverage percentage
species_zone_contributions <- dbGetQuery(
  con_sdm,
  glue(
    "
  WITH zone_cells AS (
    SELECT cell_id, pct_covered
    FROM zone_cell
    WHERE zone_seq = {z_seq}
  )
  SELECT 
    m.taxa,
    SUM(mc.val * zc.pct_covered) / SUM(zc.pct_covered) as avg_suitability,
    COUNT(DISTINCT mc.cell_id) as n_cells
  FROM zone_cells zc
  JOIN model_cell mc ON zc.cell_id = mc.cell_id
  JOIN model m ON mc.mdl_seq = m.mdl_seq
  WHERE mc.val > 0
  GROUP BY m.taxa"
  )
  ...
  left_join(d_rl, by = "taxa") |>
  mutate(
    contribution = avg_suitability * rl_score / 100
  ) # suitability is 0-100

# calculate total extinction risk per species category for the zone
sp_cat_zone_totals <- species_zone_contributions |>
  group_by(sp_cat) |>
  summarize(
    extrisk_weighted = sum(contribution, na.rm = TRUE),
    n_species = n(),
    avg_n_cells = mean(n_cells),
    .groups = "drop"
  )

# get average primary productivity for the zone
avg_prim_prod <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT 
    SUM(c.prim_prod_mean * zc.pct_covered) / SUM(zc.pct_covered) as avg_prim_prod
  FROM zone_cell zc
  JOIN cell c ON zc.cell_id = c.cell_id
  WHERE zc.zone_seq = {z_seq}
    AND c.prim_prod_mean IS NOT NULL"
  )
)$avg_prim_prod

# get the zone metric values
zone_metrics <- dbGetQuery(
  con_sdm,
  glue(
    "
  SELECT 
...
# calculate percent contribution of each component to the final score
score_value <- zone_metrics |> filter(metric_key == m_key) |> pull(val)
component_values <- zone_metrics |> filter(metric_key != m_key)
component_contributions <- component_values |>
  mutate(
    percent_contribution = round(
      val / nrow(component_values) / score_value * 100,
...
# create detailed species table
species_zone_table <- species_zone_contributions |>
  filter(!is.na(rl_score)) |>
  arrange(desc(contribution)) |>
  mutate(
    percent_of_total_extrisk = round(contribution / sum(contribution) * 100, 2)
  ) |>
  select(
    taxa,
    scientific_name_dataset,
    ...
    n_cells,
    contribution,
    percent_of_total_extrisk
  ) |>
  head(20) # top 20 species

print(glue("Planning Area: {planarea_key}"))
print(glue("Zone score: {round(score_value, 1)}"))
print(glue("Number of cells: {nrow(cells_in_zone)}"))
print(glue("Average primary productivity: {round(avg_prim_prod, 1)}"))

print("\nComponent contributions to zone score:")
print(component_contributions)

print("\nSpecies category summary:")
print(sp_cat_zone_totals)

print("\nTop species contributions to extinction risk:")
print(species_zone_table)
```
...
# convert to raster
r_m <- create_raster_from_df(d_m, r_cell, value_col = "val")
...
d <- dbGetQuery(
  con_sdm,
  "SELECT * FROM duckdb_tables() WHERE table_name = 'model_cell';"
)
...
# $ sql                    <chr> "CREATE TABLE model_cell(cell_id INTEGER, mdl_seq INTEGER, \"val\" INTEGER…
...
for (pq in dir_ls(dir_pq, glob = "*.parquet")[-1]) {
  # pq = dir_ls(dir_pq, glob = "*.parquet")[1]
...
seqs_exist <- dbGetQuery(con_sdm, "SELECT * FROM duckdb_sequences()") |>
  pull(sequence_name)

schema_sql <- glue("{dir_pq}/schema.sql")
for (ln in readLines(schema_sql)) {
  # ln = readLines(schema_sql)[1]
  if (str_detect(ln, "CREATE TABLE")) {
    ...
    sql <- glue(
      "ALTER TABLE {tbl} ALTER COLUMN {col} SET DEFAULT nextval('{seq}');"
    )
    message(sql)
...
dbExecute(
  con,
  "ALTER TABLE public.ply_planareas_2025 ADD COLUMN planarea_id SERIAL PRIMARY KEY;"
)

dbExecute(
  con,
  "ALTER TABLE public.ply_ecoregions_2025 ADD COLUMN ecoregion_id SERIAL PRIMARY KEY;"
)
```
