...
if (dbExistsTable(con_sdm, "model_cell") || dbExistsTable(con_sdm, "sdm_values")) {
  tables_to_drop <- c(
    "zone_metric", "zone_cell", "cell_metric", "env_layers", 
...
d_species <- tbl(con_sdm, "species") |> 
  ...
  mutate(
    sp_cat = case_when(
      ds_key == "am_0.05" & 
        class %in% c(
        "Actinopterygii", "Chondrichthyes", "Elasmobranchii", "Holocephali", 
        "Cephalaspidomorphi", "Myxini")   ~ "fish",
      ds_key == "am_0.05" & 
        class %in% c(
        "Elasmobranchii")                 ~ "shark", # TODO: add shark (explain, rays, skates)
      ds_key == "am_0.05" &
...
dbExecute(con_sdm, "ALTER TABLE species ALTER COLUMN sp_seq SET default nextval('species_sp_seq_seq');")
...
# display results
results <- dbGetQuery(con_sdm, glue("
  SELECT 
...
d_rl <- tbl(con_sdm, "species") |> 
  select(taxa, sp_key, sp_cat, scientific_name_dataset, common_name_dataset, redlist_code) |> 
  filter(
    !is.na(redlist_code),
    !redlist_code %in% c("DD","EX")) |> 
  collect() |> 
...
species_contributions <- dbGetQuery(con_sdm, glue("
  SELECT 
    m.taxa,
  ...
  mutate(
    contribution = suitability * rl_score / 100) |>   # suitability is 0-100
  tibble() |> 
...
prim_prod <- dbGetQuery(con_sdm, glue("
  SELECT prim_prod_mean 
  FROM cell 
  WHERE cell_id = {cell_id}"))$prim_prod_mean

# get the actual metric values for this cell
cell_metrics <- dbGetQuery(con_sdm, glue("
  SELECT 
    m.metric_key,
    cm.value
  FROM cell_metric cm
...
component_contributions <- component_values |> 
  mutate(
    percent_contribution = round(value / nrow(component_values) / score_value * 100, 1))

# create detailed species table  
species_table <- species_contributions |> 
  # filter(!is.na(rl_score)) |> 
  mutate(
    pct_total_extrisk = round(contribution / sum(contribution, na.rm = T) * 100, 2),
...
cells_in_zone <- dbGetQuery(con_sdm, glue("
  SELECT 
    cell_id,
    pct_covered
  FROM zone_cell
...
species_zone_contributions <- dbGetQuery(con_sdm, glue("
  WITH zone_cells AS (
  ...
  left_join(d_rl, by = "taxa") |> 
  mutate(
    contribution = avg_suitability * rl_score / 100)  # suitability is 0-100
...
avg_prim_prod <- dbGetQuery(con_sdm, glue("
  SELECT 
  ...
  WHERE zc.zone_seq = {z_seq}
    AND c.prim_prod_mean IS NOT NULL"))$avg_prim_prod

# get the zone metric values
zone_metrics <- dbGetQuery(con_sdm, glue("
  SELECT 
    z.value as planarea_key,
    m.metric_key,
    zm.value
...
dbExecute(con, "ALTER TABLE public.ply_planareas_2025 ADD COLUMN planarea_id SERIAL PRIMARY KEY;")

dbExecute(con, "ALTER TABLE public.ply_ecoregions_2025 ADD COLUMN ecoregion_id SERIAL PRIMARY KEY;")
```
