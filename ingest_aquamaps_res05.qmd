...
```{r}
#| label: get_species_in_planareas

# get species in planning areas
spp_pa_csv   <- glue("{dir_data}/derived/aquamaps/spp_boem-planareas-2025.csv")
do_map_br    <- F
...
dir_b_spp  <- glue("{dir_data}/derived/aquamaps.org/spp_cells0.05")

d_spp_pa_tif <- d_spp_pa |> 
  arrange(desc(n_cells_pa)) |> 
  mutate(
    sp_am_tif        = glue("{dir_am_spp}/{sp_key}.tif"),
    sp_b_tif         = glue("{dir_b_spp}/{sp_key}.tif"),
    sp_am_tif_exists = file.exists(sp_am_tif),
    sp_b_tif_exists  = file.exists(sp_b_tif) )
  
# d_spp_pa_tif |> 
#   select(sp_am_tif_exists, sp_b_tif_exists) |> 
#   table(useNA = "ifany")
#                  sp_b_tif_exists
...
for (i in 1:n){  # i = 1
  ...
  r_sp_b <- as.int(round(r_sp_b * 100)) %>%
    ifel(. == 0, NA, .) |> 
    trim()
  
  writeRaster(
    r_sp_b, sp_b_tif, overwrite = T, datatype = "INT1U")
}
```
