...
ch_taxon_csv <- glue("{dir_fws}/crithab_spp_taxon.csv")
...
for (i in d_spp$rowid){  # i = 69 # C00S _Chelonia mydas_ Green sea turtle
  ...
  # rast('/Users/bbest/Library/CloudStorage/GoogleDrive-ben@ecoquants.com/My Drive/projects/msens/data/raw/fws.gov/crithab_rast/Chelonia_mydas.tif')
  # min value   :             70 
  # max value   :             90 
...
write_csv(d_rast_taxon, ch_taxon_csv)
```

```{r}
#| label: iterate_rast_to_db
d_rast4db <- read_csv(ch_taxon_csv) # 313 × 20

# table(d_rast4db$species_group, useNA = "ifany")
#       Amphibians        Arachnids            Birds      Crustaceans 
#                2                1               18                1 
# Ferns and Allies           Fishes Flowering Plants          Insects 
#               13                4              239               10 
#          Mammals         Reptiles           Snails 
#                9               15                1 

d_birds <- d_rast4db |> 
  filter(species_group == "Birds") |>  # 18 rows
  mutate(
    sp_cat = "bird")
d_notbirds <- d_rast4db |> 
  ...
  mutate(
    sp_cat = case_match(
      ...
      "Fishes"   ~ "fish",
      "Mammals"  ~ "mammal",
      "Reptiles" ~ "turtle"))       # 11 rows
d_rast4db <- bind_rows(d_birds, d_notbirds) |> 
  arrange(scientific_name_dataset)  # 313 -> 29 rows

# table(d_rast4db$sp_cat, useNA = "ifany")
# bird   fish mammal turtle 
#   18      3      3      5
...
for (i in 1:nrow(d_rast4db)){  # i = 1
  ...
  # d_sp |> glimpse()
  # $ tif                      <chr> "/Users/bbest/My Drive/projects/msens/data…
  # $ sp_code                  <chr> "E04W"
  # $ esa_code_max             <chr> "TN"
  # $ n_esa_tn                 <dbl> 1
  # $ n_esa_en                 <dbl> 0
  # $ common_name_dataset      <chr> "Gulf sturgeon"
  # $ scientific_name_dataset  <chr> "Acipenser oxyrinchus (=oxyrhynchus) desot…
  # $ fws_url                  <chr> "https://ecos.fws.gov/ecp/species/651"
  # $ species_group            <chr> "Fishes"
  # $ worms_id                 <dbl> 1805509
  # $ scientific_name_worms    <chr> "Acipenser desotoi"
  # $ worms_is_marine          <lgl> TRUE
  # $ worms_is_extinct         <lgl> NA
  # $ itis_id                  <dbl> 624005
  # $ scientific_name_itis     <chr> "Acipenser oxyrinchus desotoi"
  # $ common_name_worms        <chr> NA
  # $ isPreferredName          <dbl> NA
  # $ common_name_itis         <chr> "Gulf sturgeon"
  # $ scientific_name_accepted <chr> "Acipenser desotoi"
  # $ common_name_accepted     <chr> "Gulf sturgeon"
  # $ sp_cat                   <chr> "fish"
  ...
  d_model <- tibble(
    ...
    mdl_type    = "binary",
    description = glue(
      "FWS critical habitat for {sp}, interpolated to 0.05° resolution") )
