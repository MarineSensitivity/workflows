...
```{r}
# show region keys
dbGetQuery(con, glue("SELECT {fld_key} FROM {schema}.{tbl} ORDER BY {fld_key}")) |> 
  pull(fld_key)
```
...
sum(data$width_value)

# Calculate positions
data <- data %>%
    arrange(desc(width_value)) %>%
    mutate(
        # Calculate angles for plotting
        ymax = cumsum(width_value),
...
ggplot(data) +
  ...
  coord_polar(theta = "y") +
  # Create donut hole
  xlim(c(-20, max(data$height_value))) +
  # Add center score
  annotate(
    "text", x = -20, y = 0, 
    label = "85", 
...
d_spp <- dbGetQuery(con, glue("SELECT * FROM {tbl_ply_spp} WHERE ply_tbl = '{schema}.{tbl}' AND ply_fld = '{fld_key}' AND ply_key = '{ply_key}'")) |> 
  mutate(
    taxa = factor(taxa, levels = names(where_taxa))) |>
  tibble()
