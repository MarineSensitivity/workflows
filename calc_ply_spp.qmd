...
```{r}
#| label: NEW ply_spp vs OLD ply_species
...
# ply_spp by prodiag ----
q_sel <- tbl(con, "ply_ecoblks") |>
  select(blk_ukey, cell_id, area_km2) |> 
  left_join(
    tbl(con, "ply_ecoblks") |> 
      select(blk_ukey, prodiag_ukey),
    by = "blk_ukey") |> 
  left_join(
    tbl(con, "spp_cells") |> 
      select(cell_id, sp_key, probability),
...
librarian::shelf(
  marinebon/aquamapsduckdb, exactextractr, mapview, sf, sfarrow, terra, tibble)
# devtools::load_all("~/Github/marinebon/aquamapsduckdb/")

source("~/Github/marinebon/aquamapsduckdb/inst/app/functions.R")

# ecoblock_pq      <- here("data/ecoblks/ecoblock.parquet")
# amcell_pq        <- here("data/ecoblks/amcell.parquet")
ecoblock_gpkg      <- here("data/ecoblks/ecoblock.parquet")
amcell_pq        <- here("data/ecoblks/amcell.parquet")
analysis_duckdb  <- here("data/ecoblks/analysis.duckdb")
...
if (!file.exists(ecoblock_pq)){
  ecoblock <- st_read(con, "ply_ecoblks") |> 
    select(-br_er_pa_pd_bk_key) # 309,229 features, 14 fields
  st_write_parquet(ecoblock, ecoblock_pq)
}
ecoblock <- st_read_parquet(eb_pq)

if (!file.exists(amcell_pq)){
  amcell <- st_read(con, "cells")
  
  st_write_parquet(amcell, amcell_pq)
}
amcell <- st_read_parquet(amcell_pq)
...
cells_sl <- dbGetQuery(con_dd, "SELECT * FROM pg_am.cells LIMIT 3")
dbGetQuery(con_dd, "SELECT * FROM ST_Drivers();") |> View()

# https://github.com/duckdb/duckdb-spatial/blob/main/docs/example.md
dbExecute(con_dd, glue("COPY pg_am.cells TO '{amcell_pq}'"))
dbExecute(con_dd, glue("COPY pg.ply_ecoblks TO '{ecoblock_pq}'"))
...
dbExecute(con_dd, glue("CREATE TABLE amcell AS (SELECT * EXCLUDE geom, ST_GeomFromWKB(geom) AS geom from '{amcell_pq}')"))
dbExecute(con_dd, glue("CREATE TABLE ecoblock AS SELECT * FROM '{ecoblock_pq}'"))

dbGetQuery(con_dd, glue("DESCRIBE SELECT * FROM amcell LIMIT 2"))
...
d_w <- extract(r, p_eb_sl, ID=F, weights=T)  # nrow(d_w): 80

# Calculate vector of mean December precipitation amount for each municipality
brazil$mean_dec_prec <- exact_extract(r, p_eb_sl, 'mean')
