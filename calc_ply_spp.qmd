...
insert_occs <- function(key){
  ...
  o <- occ_data(taxonKey = key, limit = 100000)
  
  d <- o$data |> 
    select(!where(is.list)) |> # drop column network_keys
    clean_names()
  n <- nrow(d)
  dbWriteTable(con, "species", d, append = T, row.names = F)
  return(n)
}

d_spp_ck_occs <- d_spp_ck |> 
  mutate(
    n_occs_added = map_int(usageKey, insert_occs))
