...
if (!"spp" %in% dbListTables(con)){
  ...
  # rename columns in table spp: am_key -> sp_key_am, usageKey -> sp_key_gbif 
  ...
  if ("usageKey" %in% dbListFields(con, "occ_species"))
    dbExecute(con,"ALTER TABLE spp RENAME COLUMN \"usageKey\" TO sp_key_gbif")
...
```{r}
librarian::shelf(
  dplyr, glue, here, purrr, readr, rgbif, rredlist, tictoc, utils)
...
iucn_token_txt <- glue("{dir_private}/iucnredlist.org_api_token.txt")
...
if ("taxonKey" %in% dbListFields(con, "occ_species"))
  dbExecute(
    con,
    "ALTER TABLE occ_species RENAME COLUMN \"taxonKey\" TO sp_key_gbif")

# d_occ_spp <- tbl(con, "spp") |> 
#   select(sp_key_gbif) |> 
#   left_join(
#     tbl(con, "occ_species") |> 
#       distinct(sp_key_gbif, iucnRedListCategory),
#     by = "sp_key_gbif") |> 
#   collect() |> 
...
dbExecute(
  ...
  "UPDATE spp
    ...
    FROM occ_species 
    WHERE occ_species.sp_key_gbif = spp.sp_key_gbif")
...
d_spp_rlna <- tbl(con, "spp") |> 
  ...
  select(sp_key_gbif, species) |> 
  arrange(species) |> 
  collect()
# nrow(d_spp_rlna) # 8,538

d_spp_rl <- tbl(con, "spp") |> 
  distinct(species) |> 
  arrange(species) |> 
  collect() |> 
  mutate(
    iucn_list = imap(
      species, \(sp, idx){
        message(glue("{idx}/{nrow(d_spp_rlna)}: {sp} ~ {Sys.time()}"))
        
        y <- rl_search(sp, region = "global")
        # message(str(y))
        if (length(y$result[]) == 0){
          message(glue("  not found, skipping", .trim = F))
          return(NA)
        }
        
        d <- y$result |> 
          rename(sp_id_iucn = taxonid) |> 
          mutate(species_spp = sp) |> 
          relocate(species_spp)
        n <- nrow(d)
        message("  adding to iucn_species", .trim = F)
        # append to database
        dbWriteTable(con, "iucn_species", d, append = T, row.names = F)
        return(n)

      }))
#  1/3267: Aaptos bergmanni ~ 2025-01-02 16:47:52.678079
# 78/3267: Alionematichthys piger ~ 2025-01-02 16:48:52.679629
# (2 min / 78 spp) * 3267 spp = 83.8 min
# https://docs.ropensci.org/rredlist/
# https://apiv3.iucnredlist.org/api/v3/docs#species-category

d_spp_rl3 |> 
  select(sp_key_gbif, species, iucn_list) |> 
  unnest_wider(iucn_list)

  rredlist::rl_sp_category(
    d_spp_rl$species[1])
  
  rl_
  rredlist::rl_habitats()
  rredlist::rl_synonyms()
  names(d_spp_rl)
    rl_search(d_spp_rl$verbatim_name[1])
    rl_search(d_spp_rl$species[1])
    rl_synonyms(d_spp_rl$species[1])
    rl_search(
    genus   = "Aaptos",
    species = "bergmanni",
    key     = iucn_token)
...
# rredlist: 
librarian::shelf(rredlist)
rl_use_iucn()

stopifnot(sum(duplicated(d_occ_spp$sp_key_gbif)) == 0)
