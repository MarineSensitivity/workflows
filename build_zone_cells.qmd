...
# The gate needs a RELEASED zone_cell to compare against. A version that has not
# shipped yet (v9 while it is being built) has none -- and "skipped" is exactly the
# failure mode this notebook exists to prevent. The extraction is (geometry x grid),
# so gate against ver_prev's released zone_cell on the same grid instead.
released <- function(v)
  file.exists(path.expand(glue("{dir_big}/{v}/marine-atlas/tables/zone_cell.parquet"))) ||
  v %in% msens::atlas_versions()$ver
gate_ver <- if (released(ver)) ver else
  if (identical(msens::grid_for_ver(ver_prev), msens::grid_for_ver(ver)) && released(ver_prev)) ver_prev else
  NA_character_
stopifnot("no released zone_cell to gate against (neither this version nor ver_prev on the same grid)" =
            !is.na(gate_ver))
zs_pa <- zone_sets |>
  filter(zone_type == "programarea",
         grepl(paste0("\\b", gate_ver, "\\b"), dplyr::coalesce(versions, ""))) |>
  slice_head(n = 1)
stopifnot("no programarea zone set records the gate version" = nrow(zs_pa) == 1)
f_v8 <- glue("{dir_zc}/{zs_pa$zone_set_key}/{msens::grid_for_ver(gate_ver)}/zone_cell.parquet")
cat(glue("gate: {zs_pa$zone_set_key} x {msens::grid_for_ver(gate_ver)} against released {gate_ver}\n\n"))
...
tbl_src <- function(t) {
  # the release stages tables under marine-atlas/tables/ (laptop); big/{ver}/tables/ is the server layout
  loc <- path.expand(c(glue("{dir_big}/{gate_ver}/marine-atlas/tables/{t}.parquet"),
                       glue("{dir_big}/{gate_ver}/tables/{t}.parquet")))
  loc <- loc[file.exists(loc)]
  if (length(loc)) loc[1] else glue("{msens::atlas_base_url()}/{gate_ver}/tables/{t}.parquet")
}
...
if (file.exists(f_v8)) {
  ...
  cat(glue("old (released {gate_ver}): {nrow(old)} rows, {n_distinct(old$zone_key)} zones\n",
  ...
  cat(glue("PASS — the relocated zone_cell reproduces {gate_ver}'s byte for byte\n"))
} else {
