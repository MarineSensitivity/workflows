...
d_sr_pra <- tbl(con_sdm, "zone_cell") |>
  inner_join(
    tbl(con_sdm, "zone") |> filter(tbl == "ply_subregions_2025"),
    by = "zone_seq") |>
...
sf_walrus <- sf_ranges |> filter(sci_name == sp_sci)

message(glue("IUCN walrus range: {nrow(sf_walrus)} polygon(s)"))

# get current walrus SDM values
d_walrus_before <- tbl(con_sdm, "model_cell") |>
  filter(mdl_seq == walrus_mdl_seq) |>
  collect()
n_before <- nrow(d_walrus_before)
message(glue("Walrus SDM cells before masking: {n_before}"))
# Walrus SDM cells before masking: 139,076

# create raster and mask by IUCN range
r <- init(r_cell[[1]], NA)
r[d_walrus_before$cell_id] <- d_walrus_before$value
names(r) <- "value"
# sum(!is.na(values(r)))

# mask by IUCN range (need to rotate to 0-360 longitude for consistency)
r_masked <- mask(rotate(r), vect(st_union(sf_walrus)))

# extract masked cell values
d_masked <- terra::as.data.frame(r_masked, cells = T, na.rm = T, xy = F) |>
  tibble() |>
  rename(cell_id = cell)

n_after <- nrow(d_masked)
message(glue("Walrus SDM cells after masking: {n_after} ({round((1 - n_after/n_before) * 100, 1)}% removed)"))
# Walrus SDM cells after masking: 84451 (39.3% removed)
...
dbWriteTable(con_sdm, "model_cell", d_insert, append = TRUE)
...
tbl(con_sdm, "metric") |>
  ...
  metric_seq metric_key                         description                                     
#        <int> <chr>                              <chr>                                           
# 1        303 extrisk_all_ecoregion_min          Extinction risk for all Ecoregional min         
# 2        305 extrisk_other_ecoregion_min        Extinction risk for other Ecoregional min       
# 3        307 extrisk_coral_ecoregion_min        Extinction risk for coral Ecoregional min       
# 4        309 extrisk_fish_ecoregion_min         Extinction risk for fish Ecoregional min        
# 5        311 extrisk_invertebrate_ecoregion_min Extinction risk for invertebrate Ecoregional min
# 6        313 extrisk_mammal_ecoregion_min       Extinction risk for mammal Ecoregional min      
# 7        315 extrisk_turtle_ecoregion_min       Extinction risk for turtle Ecoregional min      
# 8        317 extrisk_bird_ecoregion_min         Extinction risk for bird Ecoregional min 
```
...
#   metric_seq metric_key                              description                                          
#        <int> <chr>                                   <chr>                                                
# 1        319 extrisk_all_ecoregion_rescaled          Extinction risk for all, rescaled to [0,100] based o…
# 2        320 extrisk_other_ecoregion_rescaled        Extinction risk for other, rescaled to [0,100] based…
# 3        321 extrisk_coral_ecoregion_rescaled        Extinction risk for coral, rescaled to [0,100] based…
# 4        322 extrisk_fish_ecoregion_rescaled         Extinction risk for fish, rescaled to [0,100] based …
# 5        323 extrisk_invertebrate_ecoregion_rescaled Extinction risk for invertebrate, rescaled to [0,100…
# 6        324 extrisk_mammal_ecoregion_rescaled       Extinction risk for mammal, rescaled to [0,100] base…
# 7        325 extrisk_turtle_ecoregion_rescaled       Extinction risk for turtle, rescaled to [0,100] base…
# 8        326 extrisk_bird_ecoregion_rescaled         Extinction risk for bird, rescaled to [0,100] based …
...
# NEWEST:
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.7705  8.9566 20.8480 26.1353 39.8928 91.2885 
```
...
```{r calc_ecoregion_score_from_ecoregion_metrics}
...
d <- tbl(con_sdm, "zone") |> 
  ...
  #   value == "AKL48") |>
  select(zone_seq, zone_fld = fld, zone_value = value) |> # zone_tbl = tbl, 
  inner_join(
    tbl(con_sdm, "zone_cell") |>
...
## Load Program Areas to PostGIS

```{r pg_load_programareas}
#| eval: false
...
if (!dbExistsTable(con, "ply_programareas_2026")) {
  # load from GeoPackage and write to PostGIS
  pra <- read_sf(pra_gpkg)
  st_write(pra, con, "ply_programareas_2026", delete_layer = TRUE)
  message("Created ply_programareas_2026 in PostGIS")
}
