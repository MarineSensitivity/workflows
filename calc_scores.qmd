...
tbl <- glue("ply_planareas_2025{v_sfx}")
...
tbl <- glue("ply_ecoregions_2025{v_sfx}")
...
update_table_with_clear(
  con,
  d_pa_metrics,
  glue("ply_planareas_2025{v_sfx}"),
  "planarea_key"
)
update_table_with_clear(
  con,
  d_er_metrics,
  glue("ply_ecoregions_2025{v_sfx}"),
  "ecoregion_key"
)
```
...
if (!dbExistsTable(con, glue("ply_programareas_2026{v_sfx}"))) {
...
update_table_with_clear(
  con,
  d_pra_metrics,
  glue("ply_programareas_2026{v_sfx}"),
  "programarea_key"
)
```
...
if (!file.exists(pa_gpkg)) {
  ...
  st_read(con, glue("ply_planareas_2025{v_sfx}")) |>
...
if (!file.exists(pra_gpkg)) {
  ...
  st_read(con, glue("ply_programareas_2026{v_sfx}")) |>
...
if (!file.exists(er_gpkg)) {
  ...
  st_read(con, glue("ply_ecoregions_2025{v_sfx}")) |>
...
# TODO: manage different metrics_2026_tif versions
if (!file.exists(metrics_2026_tif)) {
...
if (!file.exists(metrics_akl48_tif)) {
  ...
  # get integer codes for AKL48 program areas from the category table
  ct <- cats(r_pa)[[1]]
  # first column is the integer code (may be named 'id' or 'value')
  akl48_ids <- ct[[1]][ct$programarea_key %in% pra_sr]

  # strip categories to get raw integer raster, then mask
  r_pa_int <- r_pa
  levels(r_pa_int) <- NULL
  r_mask <- ifel(r_pa_int %in% akl48_ids, 1, NA)
  # plot(r_mask)
