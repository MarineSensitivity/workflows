...
pra_gpkg_raw         <- glue("{dir_raw}/boem.gov/boem_program_areas_2025-11.gpkg")
...
pa_boem_geo          <- glue("{dir_raw}/boem.gov/OCS_Proposed_Planning_Area_Polygons.geojson")
pa_metrics_geo       <- glue("{dir_derived}/OCS_Proposed_Planning_Area_Polygon_Env_Sensitivity_Metrics.geojson")
  ...
  pra <- read_sf(pra_gpkg_raw) |>
    ...
    mutate(
      ...
      region_key = case_when(
        ...
        TRUE ~ region_key)) |> 
    select(programarea_id, region_key, region_name, planarea_key, planarea_name, programarea_key, programarea_name) |> 
...
dbExecute(con, glue("UPDATE {tbl} SET geom = ST_MakeValid(geom) WHERE NOT ST_IsValid(geom)")) # 6 rows fixed
...
```{r subregion_programareas_lookup}
#| eval: false
...
message(glue("Wrote {nrow(d_sr_pra)} subregion-programarea mappings to {sr_pra_csv}"))
...
r_w <- get_rast_con(walrus_mdl_seq, con_sdm_2025) |> 
  rotate()
n_before <- global(r_w, 'notNA')
message(glue("Walrus SDM cells before masking: {n_before}")) # 139,076

# load IUCN marine mammal ranges
...
sf_walrus <- read_sf(marmam_sealand_shp) |>
  filter(sci_name == sp_sci) |> 
  # st_shift_longitude() |>  # shift lon [-180, 180] to [0, 360] to match raster
  st_union() # 2 polygons

# mask by IUCN range
r_w_m <- mask(r_w, vect(sf_walrus))

n_after <- global(r_w_m, 'notNA')
message(glue("Walrus SDM cells after masking: {n_after}")) # 84,451
...
# get tibble
d_w_mc <- terra::as.data.frame(r_w_mc, cells = F, na.rm = T, xy = F) |>
  tibble()

message(glue("Walrus SDM cells after masking: {n_after} ({round((1 - n_after/n_before) * 100, 1)}% removed)"))
# Walrus SDM cells after masking: 84,451 (39.3% removed)

# update database: delete and re-insert masked values
dbExecute(con_sdm, glue("DELETE FROM model_cell WHERE mdl_seq = {walrus_mdl_seq}"))

d_model_cell_w <- d_w_mc |>
  mutate(mdl_seq = walrus_mdl_seq) |>
  select(mdl_seq, cell_id, value)
dbWriteTable(con_sdm, "model_cell", d_model_cell_w, append = TRUE)

message(glue("Updated model_cell table for walrus (mdl_seq = {walrus_mdl_seq})"))
# Updated model_cell table for walrus (mdl_seq = 790)
```
...
```{r cell_metrics_to_programarea_metrics}
#| eval: false
...
sp_cats_u <- c("all", "bird", "coral", "fish", "invertebrate", "mammal", "other", "turtle")
metrics <- c(
...
```{r calc_planarea_score_from_planarea_metrics}
...
d_sr <- tribble(
  ...
  "GA",           "Gulf of America",
  "PA",           "Pacific",
  "USA",          "All USA") # note: subregion_name not used (yet)
...
df_z_sr_usa <- df_z_sr_notusa |> 
  ...
  mutate(
    ...
    pct_covered = pmin(pct_covered, 100, na.rm = T)) # 349,139 × 3
# summary(df_z_sr_usa)
...
if (!"ply_subregions_2025" %in% zone_tbl){
  ...
  d_sr <- terra::as.data.frame(
    r_sr, cells = T, na.rm = F, wide = F) |> 
    filter(
      !is.na(values)) |>  # 839,534 × 3
    tibble()  
...
tbl_taxon <- tbl(con_sdm, "taxon") |>
  ...
  # collect()  # OLD PlanAreas_2025: 17,379 × 8; NEW ProgramAreas_2026: 17,353 × 8
...
d <- tbl(con_sdm, "zone") |> 
  ...
  inner_join(
    ...
    by = join_by(cell_id)) |>
  group_by(
    zone_tbl, zone_fld, zone_value,  # zone_seq,
    mdl_seq,
...
update_table_with_clear(con, d_pra_metrics, "ply_programareas_2026", "programarea_key")
```
...
dbExecute(con, "ALTER TABLE public.ply_programareas_2026 ALTER COLUMN programarea_id TYPE INTEGER;")
...
if (!file.exists(metrics_2026_tif)) {
  ...
  for (zone_fld in c("ecoregion_key", "programarea_key")){ # zone_fld = "ecoregion_key"
...
```{r r_metrics_akl48_primprod_log}
...
# pa_metrics_geo  <- glue("{dir_data}/derived/OCS_Proposed_Planning_Area_Polygon_Env_Sensitivity_Metrics.geojson")
  ...
  pa_metrics_geo <- glue("{dir_data}/derived/OCS_Area_Polygons_env_sensitivity_metrics.geojson")
...
# import parquet ----
for (pq in dir_ls(dir_pq, glob = "*.parquet")) { # pq = dir_ls(dir_pq, glob = "*.parquet")[2]
