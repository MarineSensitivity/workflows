...
- **Cell-level endemism / species richness**: Currently species richness is
  implicitly captured by summing species layers per taxonomic group. A future
  enhancement could compute explicit richness or cell-level endemism metrics.
- **Endemism vs richness distinction**: "Endemism" (range restriction) is a
  species-level property; "species richness" (how many species share a cell)
  is a cell-level property. The former is computed in `ms_endemism` per species;
  the latter emerges naturally from summing `ms_merge` layers across species.
...
) # local: big files outside GDrive
dir_create(dir_v)
dir_create(dir_big)
...
for (sp_cat in sp_cats) {
  ...
  # ms_merge values already incorporate extinction risk per cell
  # (ms_merge = max(1, round(ms_suit * ms_er / 100))), so we simply
  # sum ms_merge values across species — no scalar er_score multiplication
  d_mdl <- tbl(con_sdm, "taxon") |>
    filter(
      is_ok,
  ...
  duckdb_register(con_sdm, "tmp_mdl", d_mdl)
  dbExecute(
    con_sdm,
    glue(
      "
    ...
    FROM model_cell mc
    JOIN tmp_mdl tm ON mc.mdl_seq = tm.mdl_seq
    GROUP BY mc.cell_id"
    )
  )
  duckdb_unregister(con_sdm, "tmp_mdl")
...
# ms_merge values already incorporate ER per cell, so avg_suit from
# ms_merge is the combined suitability x extinction risk score.
# suit_rl is simply the avg ms_merge value scaled to 0-1.
d <- d |>
  mutate(
    suit_rl = avg_suit,
    suit_rl_area = avg_suit * area_km2
  ) |>
