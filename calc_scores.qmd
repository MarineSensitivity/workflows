...
# flags, typical
do_zones <- T # no, except if new spatial or change versions
do_metrics <- T # YES
...
stopifnot(!do_pmtiles && do_downloads) # pmtiles should be generated from downloads
...
# delete old versioned zones (program areas, subregions) — will be recreated
for (old_tbl in zone_tbls_db[str_detect(zone_tbls_db, glue("_{ver_prev}$"))]) {
  z_seqs <- tbl(con_sdm, "zone") |>
    filter(tbl == !!old_tbl) |>
    pull(zone_seq)
  if (length(z_seqs) > 0) {
    seq_list <- paste(z_seqs, collapse = ",")
    dbExecute(
      con_sdm,
      glue("DELETE FROM zone_metric WHERE zone_seq IN ({seq_list})")
    )
    dbExecute(
      con_sdm,
      glue("DELETE FROM zone_cell   WHERE zone_seq IN ({seq_list})")
    )
    dbExecute(
      con_sdm,
      glue("DELETE FROM zone        WHERE zone_seq IN ({seq_list})")
    )
    # zone_taxon uses zone_tbl column
    dbExecute(
      con_sdm,
      glue("DELETE FROM zone_taxon  WHERE zone_tbl = '{old_tbl}'")
    )
    message(glue("Deleted old zone: {old_tbl} ({length(z_seqs)} zones)"))
  }
}
...
for (is_ak in c(T, F)) {
  ...
  er_url <- glue(
    "https://api.marinesensitivity.org/tilejson?table=public.{tbl_er}&use_cache=FALSE"
  )
  pra_url <- glue(
    "https://api.marinesensitivity.org/tilejson?table=public.{tbl_pra}&use_cache=FALSE"
  )
...
# previous version score csv
pra_n_csv_prev <- glue(
  "{dir_derived}/{ver_prev}/tbl_pra_scores_{ver_prev}.csv"
)
comp_csv <- glue("{dir_v}/tbl_pra_scores_{ver}_vs_{ver_prev}.csv")

if (file_exists(pra_n_csv_prev) && file_exists(pra_n_csv)) {
  d_prev <- read_csv(pra_n_csv_prev) |>
    rename(`Area (1,000 km2)` = `Area (1,000 km^2^)`)
  d_curr <- read_csv(pra_n_csv) |>
    rename(`Area (1,000 km2)` = `Area (1,000 km^2^)`)
  ...
  # summary table: wide format with diff per component
  d_comp_wide <- d_comp |>
  ...
  d_comp_wide |>
    DT::datatable(
      caption = glue("Score differences ({ver} - {ver_prev})"),
      options = list(pageLength = 25, dom = "t")
    ) |>
    DT::formatRound(columns = comp_cols, digits = 0)
} else {
  message(glue(
    "Skipping score comparison: ",
    "prev={file_exists(pra_n_csv_prev)}, ",
    "curr={file_exists(pra_n_csv)}"
  ))
}
```
