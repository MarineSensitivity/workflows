...
This should be run after `merge_models.qmd`.
...
pa_gpkg <- glue("{dir_derived}/ply_planareas{v_sfx}.gpkg") # for generating program areas

# outputs
sr_pra_csv <- glue("{dir_derived}/subregion_programareas.csv")
pra_gpkg <- glue("{dir_derived}/ply_programareas{v_sfx}.gpkg")
er_gpkg <- glue("{dir_derived}/ply_ecoregions{v_sfx}.gpkg")
sr_gpkg <- glue("{dir_derived}/ply_subregions{v_sfx}.gpkg")
metrics_tif <- glue("{dir_derived}/rast_metrics{v_sfx}.tif")
metrics_pra_tif <- glue("{dir_derived}/rast_metrics_programareas{v_sfx}.tif")
zone_taxon_csv <- glue("{dir_derived}/zone_taxon{v_sfx}.csv")
lyrs_csv <- glue("{dir_derived}/layers{v_sfx}.csv")
...
# note: Postgres db `con` set by `source(here("libs/db.R"))` below
...
if (!glue("ply_programareas_2026{v_sfx}") %in% zone_tbls) {
  source(here("libs/sdm_functions.R"))
...
```{r}
#| label: calc_programrea_score_from_programarea_metrics
...
sp_cats_u <- sort(unique(d_taxon_ok$sp_cat))
...
```{r}
#| label: pg_update_programarea_ecoregion_metrics
...
### Re-add cell table to DuckDB if missing (for generating metrics rasters)
