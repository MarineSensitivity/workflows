...
librarian::shelf(
  cmocean, DT, fs, glue, here, htmltools, janitor, jsonlite, leaflet, mapview, 
  rerddap, R.utils, sf, stringr, terra, units)
...
plet(r)
mapView(r, col.regions = cmocean("algae"))
```
...
pa_oa_npp <- pa_oa |>
  mutate(
    npp_cellavg = zonal(r_r, pa_oa |> vect(), fun = "mean", na.rm = T) |>
      pull(monthly_npp_mean))

(mapView(r, col.regions = cmocean("algae"), layer.name = "raster") + 
  mapView(pa_oa_npp, zcol = "npp_cellavg", col.regions = cmocean("algae"), layer.name = "planning areas")) |> 
  slot("map") |> 
   addControl(tags$div(HTML("units: mg C / m<sup>2</sup> / day")), position = "bottomright")
