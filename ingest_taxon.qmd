...
dir_worms    <- glue("{dir_data}/raw/marinespecies.org/worms_2025-07-01_checklistbank.org")
...
if (!"worms" %in% dbListTables(con_spp)){
  ...
  d_habs <- vroom(
    ...
    mutate(across(
      c(taxonID),
      ~ str_replace(.x, "urn:lsid:marinespecies.org:taxname:", "") |> 
        as.integer() )) # 1,484,358 × 6
  d_worms <- d_worms |> 
    left_join(
      d_habs,
      by = "taxonID") # 1,484,358 × 37
...
if (any(!file_exists(c(botw_csv,botw_vern_csv)))){
  ...
  botw <- d_botw |>
    ...
    select(
      ...
      -common_name,
      -common_name_alternatives) # 11,195
  ...
  botw_vernacular_alt <- d_botw |>
    ...
    mutate(
      ...
      vernacularName = vernacularName |> 
        str_squish() |> 
        str_replace_all(" , ", ", ")) |>                      # 4,770
    separate_longer_delim(
      vernacularName, delim = ", ") |>                        # 8,190
    filter(
      !duplicated(vernacularName),                            # 8,151
      !(vernacularName %in% botw_vernacular$vernacularName))  # 7,967

  stopifnot(length(setdiff(names(botw_vernacular), names(botw_vernacular_alt))) == 0)
  stopifnot(length(setdiff(names(botw_vernacular_alt), names(botw_vernacular))) == 0)
...
if (!"botw" %in% dbListTables(con_spp)){
  ...
  dbExecute(con_spp, "CREATE INDEX botw_acceptedNameUsageID_idx on botw (acceptedNameUsageID)")
  dbExecute(con_spp, "CREATE INDEX botw_scientificName_idx on botw (scientificName)")
...
if (!"botw_vernacular" %in% dbListTables(con_spp)){
  ...
  dbWriteTable(con_spp, "botw_vernacular", botw_vernacular, append = F, batch_rows = 10000)
  
  dbExecute(con_spp, "CREATE INDEX botw_vernacular_taxonID_idx on botw_vernacular (taxonID)")
}
