...
1. component → zone = `SUM(cm.value × pct_covered) / SUM(pct_covered)`
2. PA pct_area down-weight: `value × (data_coverage / total_coverage)` (backs up `_prepctareaweighting`)
3. PA score = `SUM(value)/COUNT(value)` (plain mean) of the pct_area-weighted components
...
for (k in comp_keys) {
  ...
  dbExecute(con, glue("INSERT INTO zone_metric (zone_seq, metric_seq, value)
    SELECT zc.zone_seq, {bs}, SUM(cm.value*zc.pct_covered)/SUM(zc.pct_covered)
    FROM zone_cell zc JOIN zone z USING(zone_seq) JOIN cell_metric cm ON zc.cell_id=cm.cell_id
    WHERE z.fld='programarea_key' AND cm.metric_seq={bs} AND cm.value IS NOT NULL
    GROUP BY zc.zone_seq"))
  # backup pre-weight value
  dbExecute(con, glue("INSERT INTO zone_metric (zone_seq, metric_seq, value)
    SELECT zone_seq, {bk}, value FROM zone_metric WHERE metric_seq={bs}
      AND zone_seq IN (SELECT zone_seq FROM zone WHERE fld='programarea_key')"))
  # 2b pct_area = (coverage of cells with data for this metric) / (coverage of all cells in zone); value *= pct_area
  dbExecute(con, glue("UPDATE zone_metric zm SET value = value * (
      SELECT SUM(zc.pct_covered) FILTER (WHERE cm.value IS NOT NULL) * 1.0 / SUM(zc.pct_covered)
...
dbExecute(con, glue("INSERT INTO zone_metric (zone_seq, metric_seq, value)
  SELECT zone_seq, {sc}, SUM(value)/COUNT(value)
  FROM zone_metric
  WHERE metric_seq IN ({seqs_sql}) AND value IS NOT NULL
    AND zone_seq IN (SELECT zone_seq FROM zone WHERE fld='programarea_key')
  GROUP BY zone_seq"))
v8_pa <- dbGetQuery(con, glue("SELECT z.value AS programarea_key, round(zm.value,2) AS score_v8
