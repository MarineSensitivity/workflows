---
title: "Score zone metrics — program-area composite + previous-version equivalence gate"
msens:
...
flowchart LR
  ...
  zm --> gate["pra_score_delta<br/>{ver} vs {ver_prev} gate"]
...
v8_pa <- dbGetQuery(con, glue("SELECT z.val AS programarea_key, round(zm.val,2) AS score_new
...
v7_pa <- st_read(pa_gpkg, quiet=TRUE) |> st_drop_geometry() |>
  transmute(programarea_key, score_prev = round(score_extriskspcat_primprod_ecoregionrescaled_equalweights, 2))
cmp <- v8_pa |> inner_join(v7_pa, by="programarea_key") |>
  mutate(delta = round(score_new - score_prev, 2)) |> arrange(desc(abs(delta)))
msens::report_table(cmp, caption = glue("program-area score: {ver} (score_new) vs {ver_prev} (score_prev)"))
gate <- with(cmp, list(n=nrow(cmp), mean_abs=round(mean(abs(delta)),2), max_abs=round(max(abs(delta)),2),
  rmse=round(sqrt(mean(delta^2)),2), cor=round(cor(score_new, score_prev),3)))
str(gate)
