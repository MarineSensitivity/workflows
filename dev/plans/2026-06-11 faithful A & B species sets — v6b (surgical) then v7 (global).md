# Plan: faithful A & B species sets — v6b (surgical) then v7 (global)
...
Delivered in two stages: **(1) v6b** — surgically drop the Program-Area gate on the existing v6 data to
expose Set A immediately (no re-ingest, low risk, validates the A/B split); **(2) v7** — the full global
re-ingest. Outcome: both **A** (entire study area / global biodiversity) and **B** (within the 20 Program Areas) are produced
...
## Proposed approach — two stages: v6b (surgical) then v7 (global)

Core principle (both stages): **`is_ok` = taxonomic/data validity + has a modeled distribution in the
study area (US extent in v6b, global in v7). Program-Area / region membership is a SPATIAL query (zones),
...
## STAGE 1 — v6b: surgically remove the Program-Area gate (no re-ingest)
...
flag hides them. Fast, low-risk, validates the A/B split before the big rewrite.
...
- **1.1 Recompute `is_ok`** (merge_models.qmd): delete the program-area narrowing (L829-859 `taxa_in_pra`
  ...
  merge re-run needed — apply as a targeted `UPDATE taxon SET is_ok = …` on a copy of v6 → new `v6b`
...
- **1.3 Scores decision (KEY, must resolve):** widening `is_ok` leaves **within-PRA raw `cell_metric`
  unchanged** (added species have no cells inside PRAs), but the **per-ecoregion rescale min/max can
  shift** (added species occupy non-PRA parts of shared US ecoregions). Decide + quantify the delta:
  (a) freeze v6 rescaling so PRA scores are byte-identical and Set A affects only species LISTS, or
  (b) rescale over the full valid set and report the (expected small) PRA score change. Diff v6b-vs-v6
  `zone_metric` for the 20 PRAs to size it.
- **1.4 Apps (US extent, stay on current rendering — no titiler migration yet):** mapgl/mapsp default to
  ...
  empty/error copy (mapsp app.R ~L538, L913). Point apps at `ver <- "v6b"`.
...
Deliverable: a `v6b` database + apps/report that show true biodiversity everywhere in the US study area,
with Program Areas as one selectable scope.

---

## STAGE 2 — v7: global re-ingest (builds on v6b's validity-only `is_ok`)
Everything below goes worldwide; carries forward the v6b principle that region scope is spatial, not a flag.
...
- **v6b**: the rescaling decision in 1.3 (freeze v6 scores vs rescale over the wider set) — resolve first,
  it gates whether v6b changes any published scores.
- **v7 (Phase 0)**: (1) global rescaling regions — FAO (recommended, already local) vs MEOW vs hybrid;
  (2) longitude convention to standardize on ([-180,180] recommended); (3) whether US/PRA scores must stay
  numerically comparable to v6/v6b (drives the rescaling choice); (4) grid storage/perf budget at ~15-20M
  global cells (DuckDB size, tile cache).
...
- **Score impact**: diff v6b-vs-v6 `zone_metric` for the 20 PRAs → confirm the 1.3 decision (either zero
  delta if frozen, or a quantified, understood shift).
...
- **Score stability**: diff v7-vs-v6b `zone_metric` for the 20 PRAs; quantify shift from the global
  rescaling-basis change and confirm intended.
...
- **End-to-end**: `quarto::quarto_render()` the ingest/calc notebooks on a small global subset (e.g. one
  FAO region) before the full global build.
