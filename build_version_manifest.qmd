...
registry_only <- Sys.getenv("MANIFEST_REGISTRY_ONLY") != ""  # publish versions.json (+ latest.txt
                                                              # under PROMOTE_LATEST=1) WITHOUT
                                                              # rebuilding this ver's manifest.json
                                                              # -- e.g. registering a new version's
                                                              # row without opening this ver's sdm_db
...
if (registry_only) {
  message(glue(
    "MANIFEST_REGISTRY_ONLY=1 - publishing versions.json only, ",
    "skipping {ver}'s manifest build (no sdm_db connection opened)"))
  mfst <- NULL
} else {
...
if (!is.null(mfst)) {
  ...
  cat("capabilities: skipped (MANIFEST_REGISTRY_ONLY=1)\n")
}
```
...
if (!is.null(mfst)) msens::validate_manifest(mfst, ver = ver)
...
if (!is.null(mfst)) {
  ...
  f_manifest <- NA_character_
  cat(glue("wrote {f_versions}\n     (manifest.json skipped: MANIFEST_REGISTRY_ONLY=1)\n"))
}
```
...
if (do_s3) {
  ...
  # MANIFEST_REGISTRY_ONLY=1 pushes the registry and NOTHING else: `*.json` would also
  # sync whatever {ver}/manifest.json an earlier run left staged here, i.e. re-publish a
  # stale manifest as a side effect of registering a version; and latest.txt goes only
  # under PROMOTE_LATEST=1, so a leftover local copy cannot move the default
  inc <- if (registry_only) c("versions.json", if (promote) "latest.txt") else c("*.json", "latest.txt")
  args <- c("s3", "sync", shQuote(path.expand(dir_reg)), shQuote(s3_root),
            "--exclude", shQuote("*"),
            as.vector(rbind("--include", shQuote(inc))),
  ...
  # (CalCOFI's index publisher learned this the expensive way). Skipped when
  # MANIFEST_REGISTRY_ONLY=1 didn't touch this ver's manifest.json this run --
  # its already-published manifest is unaffected by a registry-only push.
  if (!registry_only) {
  ...
  cat(glue("verified {if (registry_only) '' else paste0(http_root, '/', ver, '/manifest.json and ')}",
           "versions.json (access column)\n"))
} else {
...
msens::write_manifest(
  ...
  stats = list(
    ver           = ver,
    status        = if (!is.null(mfst)) mfst$status   else v_row$status,
    access        = if (!is.null(mfst)) mfst$access   else v_row$access,
    grid_id       = if (!is.null(mfst)) mfst$grid_id  else msens::grid_for_ver(ver),
    id_field      = if (!is.null(mfst)) mfst$id_field else NA_character_,
    n_versions    = nrow(versions),
    n_metrics     = if (!is.null(mfst)) nrow(mfst$metrics) else NA_integer_,
    n_zone_sets   = if (!is.null(mfst)) nrow(mfst$zones)   else NA_integer_,
    latest        = latest,
    promoted      = promote,
    registry_only = registry_only),
