...
msens:
  ...
  dataset: {ds_key: dps_nmfs, response_type: range, source_authority: NOAA NMFS, temporal_interval: static,
            ...
            value_info: "extinction-risk score per cell (NMFS:EN 100 / NMFS:TN 50 inside a listed entity's critical habitat; IUCN category elsewhere, the MMPA floor of 20 for mammals)",
...
- **Value = `compute_er_score()`**, never a magic number: `NMFS:EN` → 100, `NMFS:TN` → 50 inside a
  listed entity's polygons; elsewhere the baseline every unlisted species of its kind carries —
  `NMFS:LC` + the MMPA floor = 20 for a marine mammal (the convention `merge_models_prep` uses for
  the governing score), `IUCN:{category}` otherwise; overlaps take the max.
- **Merge:** taxa with a `dps_nmfs` model take their own branch (`msens::dps_sql`): the merged
  value is the **suitability** (AquaX / AquaMaps) masked to this ER footprint — the distribution,
  not a risk-weighted one — and the per-cell ER is written beside it (`dist_merged_er`) for
  `score_cell_metrics` to multiply in (`taxon.er_mode = 'cell'`). Critical-habitat datasets are not
  an input for these taxa — the same polygons are here, with their entity's status.
