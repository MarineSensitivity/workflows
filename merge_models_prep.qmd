...
# US ESA status stays species-specific (from the listing); is_bcc too
if ("listing" %in% dbListTables(con)) {
  tx <- tx |> left_join(dbGetQuery(con, "SELECT sci, nmfs_esa, fws_esa, is_bcc FROM listing"), by = "sci")
} else {
  log_warn("no listing table — run ingest_listings; ESA falls back to IUCN only")
  tx <- tx |> mutate(nmfs_esa = NA, fws_esa = NA, is_bcc = FALSE)
}
# TAXONOMY-BASED floors: the MMPA (marine mammals) and MBTA (birds) are TAXA-based laws,
# so assign the floor by taxonomy — NOT by an imperfect species list that mis/under-matches.
# MMPA -> all WoRMS class Mammalia (matched by AphiaID = taxon_id); MBTA -> all Aves (= BOTW).
...
tx <- tx |> mutate(
  ...
  is_mmpa = taxon_authority == "worms" & taxon_id %in% mammalia,   # class Mammalia
  is_mbta = taxon_authority == "botw",                             # class Aves
