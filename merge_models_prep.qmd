...
if ("listing" %in% dbListTables(con)) {
  tx <- tx |> left_join(
    dbGetQuery(con, "SELECT sci, nmfs_esa, fws_esa, is_mmpa, is_mbta, is_bcc FROM listing"), by = "sci")
} else {
...
log_info("governing er_score: {sum(tx$is_us)} US-listed ({sum(tx$is_mmpa)} MMPA, {sum(tx$is_mbta)} MBTA) of {nrow(tx)} taxa")
dbDisconnect(con, shutdown = TRUE)
```
