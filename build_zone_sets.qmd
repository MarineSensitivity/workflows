...
format:
  html:
    # Render mermaid CLIENT-SIDE. The project _quarto.yml still sets
    # `mermaid-format: png`, which routes every diagram through headless Chrome
    # — absent on the server (the render dies at the last step with "Chrome not
    # found", after every R chunk has already run), and per CLAUDE.md a source
    # of indefinite, unkillable hangs where Chrome IS present. `js` needs no
    # browser. Scoped to this notebook so the shared config is untouched.
    mermaid-format: js
editor_options:
