...
if (Sys.getenv("ZONE_PMTILES") != "" && !nzchar(Sys.which("tippecanoe")))
  log_warn("ZONE_PMTILES set but tippecanoe is not installed - skipping")
...
if (do_pmt) {
  ...
  for (i in seq_len(nrow(vint))) {
    ...
    log_info("{z$zone_set_key}: {round(file.info(out)$size/1024^2, 1)} MB")
  }
  ...
  if (Sys.getenv("ZONE_PMTILES_NO_S3") == "") {
    ...
    log_info("published {nrow(pmt)} zone tile set(s) to {s3_atlas}/zones")
  }
  knitr::kable(pmt |> dplyr::mutate(mb = round(file.info(pmtiles)$size/1024^2, 1)) |>
                 dplyr::select(zone_set_key, mb))
} else {
  log_info("ZONE_PMTILES unset - skipping tile build")
}
```
