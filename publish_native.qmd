...
We only publish inputs that feed a **listed taxon** (`is_valid_global & is_marine`, non
reptile/amphibian) — the set the species app actually lists. This was `is_valid_usa`, which is
NARROWER than the picker: the app lists every valid marine taxon, US or not, so a non-US species'
inputs were never built and showed as unavailable while its merged surface drew fine (6,352 of
25,404 input edges).

**Flags:** `REDO_NATIVE=1` (rebuild PMTiles + IUCN gpkg + re-sort am), `REDO_PMTILES=1` (rebuild
just the vector tiles), `NATIVE_SKIP_AM` / `NATIVE_SKIP_PMTILES`,
...
# is_valid_GLOBAL, matching the species picker. Building for is_valid_usa left every non-US
# taxon's inputs unpublished while its merged COG (built for all valid marine taxa) drew fine.
served <- dbGetQuery(con, "
  ...
  WHERE t.is_valid_global AND t.is_marine AND t.sp_cat NOT IN ('reptile','amphibian')")
...
if (!file_exists(iucn_gpkg) || redo_src) {
  ...
  if (file_exists(iucn_all)) {
    ...
    sql_i <- glue("SELECT id_no, ('rng_iucn|' || CAST(id_no AS TEXT)) AS mdl_key, ",
                  "'rng_iucn' AS ds_key FROM rng_iucn WHERE id_no IN ({ids_sql})")
    ...
    log_info("wrote {path_file(iucn_gpkg)}")
  } else {
...
if (nrow(amonly_multi) && dir_exists(am_parts)) {
  ...
  for (i in seq_len(nrow(amonly_multi))) {
    ...
    # bbox from the cells themselves, same convention as the other COG rows
    ll <- msens::cell_lonlat(d$cell_id, grid_mm)
    mm_rows[[length(mm_rows) + 1]] <- tibble(
      ...
      xmin = min(ll$lon), xmax = max(ll$lon), ymin = min(ll$lat), ymax = max(ll$lat),
      rescale_min = 1L, rescale_max = 100L, colormap = "spectral_r")
  }
  if (length(mm_rows)) {
