...
if (!skip_pmt) for (ds in pmt_ds) {
  ...
  if (file_exists(out)) pmt_rows[[ds]] <- tibble(
    ...
    # ?v={mtime} busts the browser/PMTiles-protocol cache on every rebuild (same filename,
    # new content otherwise serves stale tiles); Caddy ignores the query and serves the file.
    asset_url = glue("{pmt_base}/{ds}.pmtiles?v={as.integer(file_info(out)$modification_time)}"),
    source_layer = ds)
}
