...
if (!skip_pmt) for (ds in pmt_ds) {
  ...
  if (file_exists(out) && !redo) { log_info("{ds}.pmtiles exists — skip (REDO_NATIVE=1 to force)") }
    ...
    # z5 is plenty for coarse expert-range overlays; z6 on thousands of huge global
    # IUCN polygons runs ~30 min. maxzoom is per-dataset so a slow one can go lower.
    mz <- if (ds == "rng_iucn") 5L else 6L
    publish_pmtiles(x, out, layer = ds, minzoom = 0, maxzoom = mz)
  }
...
# bbox by mid over the compact partitioned parquet, then map mid -> mdl_key via am_map.
