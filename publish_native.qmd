...
## NMFS DPS extinction-risk surfaces → per-species COGs

The `dps_nmfs` surfaces (per-cell extinction risk: each listed entity's habitat at its status, the
species' baseline elsewhere) are *gridded from the start* — built by `ingest_nmfs-dps.qmd` from the
NMFS critical-habitat service + the IUCN range — so their one representation is the model grid
itself, painted as a COG per species. Without these rows the species app listed the input as
*NMFS DPS extinction risk* but struck it through: an input with no published surface is disabled.
Resumable by file existence; `REDO_DPS_COG=1` (or `REDO_NATIVE`) repaints.
...
bb_vec <- tibble()
dps_rows <- if (exists("dps_reg") && nrow(dps_reg)) dps_reg else tibble()
