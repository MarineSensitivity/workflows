...
librarian::shelf(dplyr, glue, knitr, logger, MarineSensitivity/msens, quiet = TRUE)
...
ix <- msens::build_storage_index(objs, site_url = site, obj_url = obj_url)
...
if (do_s3 && nrow(ix)) {
  args <- c("s3", "sync", shQuote(dir_out), shQuote(bucket),
            "--exclude", shQuote("*"), "--include", shQuote("*index.html"),
            "--content-type", shQuote("text/html"),
