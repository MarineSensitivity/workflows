...
for (t in rel_tables)
  dbExecute(cv, glue("CREATE VIEW {t} AS SELECT * FROM read_parquet('{tbl_src}/{t}.parquet')"))
...
log_info("view DB {viewdb} ({round(file_size(viewdb)/1e3,1)} KB) — {if (do_s3) 'S3' else 'LOCAL'} sources, model_cell partitioned by mdl_id (public key = mdl_key)")
```
...
if (!do_apps) {
  ...
  cmd <- glue("cd {apps_co} && git pull --quiet && ",
              "for a in {paste(v8_apps, collapse=' ')}; do touch {apps_co}/$a/restart.txt; done")
  status <- system2("ssh", c("msens", shQuote(cmd)), stdout = TRUE, stderr = TRUE)
  log_info("deployed v8 apps [{paste(v8_apps, collapse=', ')}] from apps_v8@main; v7 apps untouched")
}
```
