...
if (!do_tables) {
  ...
  system2("ssh", c("msens", glue("mkdir -p {srv_tables}")))
  system2("rsync", c("-aq", "--delete", glue("{tbl_dir}/"), glue("msens:{srv_tables}/")))
  log_info("synced tables -> msens:{srv_tables}")
  # repoint every released table view at the server's local copy, preserving the
  # `val AS value` back-compat alias the apps still rely on. Verified with a real
  # count so a half-synced directory FAILS LOUDLY instead of serving broken views.
  fix_tbls <- glue(
    "docker exec rstudio Rscript -e ",
    shQuote(glue(
      ...
      "cat('TABLES_OK', length(tt), DBI::dbGetQuery(con, ",
      "'SELECT count(*) n FROM zone_metric')$n, '\\n'); ",
      "DBI::dbDisconnect(con, shutdown = TRUE)")))
  out_tb <- system2("ssh", c("msens", shQuote(fix_tbls)), stdout = TRUE, stderr = TRUE)
  if (!any(grepl("TABLES_OK", out_tb))) stop(paste(tail(out_tb, 15), collapse = "\n"))
  log_info("table views repointed to local Parquet: {grep('TABLES_OK', out_tb, value = TRUE)}")
  ...
  system2("ssh", c("msens", glue("mkdir -p {srv_modelcell}")))
  ...
  fix_mc <- glue(
    ...
    shQuote(glue(
      ...
      "cat('MODELCELL_OK', DBI::dbGetQuery(con, ",
      "'SELECT count(*) n FROM model_cell WHERE mdl_id = (SELECT min(mdl_id) FROM model)')$n, '\\n'); ",
      "DBI::dbDisconnect(con, shutdown = TRUE)")))
  out_mc <- system2("ssh", c("msens", shQuote(fix_mc)), stdout = TRUE, stderr = TRUE)
  if (!any(grepl("MODELCELL_OK", out_mc))) stop(paste(tail(out_mc, 15), collapse = "\n"))
  log_info("model_cell view repointed to local Parquet: {grep('MODELCELL_OK', out_mc, value = TRUE)}")
}
```
...
if (!nzchar(Sys.getenv("DEPLOY_API"))) {
  ...
  cmd_api <- glue(
    ...
    "docker compose up -d --build plumber; ",
