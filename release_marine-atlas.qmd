...
if (!do_titiler) {
  ...
  for (i in seq_len(nrow(chk))) {
    js <- system2("curl", c("-s", "--retry", "3", shQuote(glue(
      "{tt_base}/cog/info?url={URLencode(chk$asset_url[i], reserved = TRUE)}"))), stdout = TRUE)
    b <- tryCatch(jsonlite::fromJSON(paste(js, collapse = ""))$bounds, error = function(e) NULL)
    if (is.null(b) || length(b) != 4) { bad <- c(bad, glue("{chk$mdl_key[i]}: no bounds")); next }
