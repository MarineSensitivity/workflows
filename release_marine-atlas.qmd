...
# GET, no LIST). This glob VIEW is for ad-hoc/statistics queries; it needs S3 LIST -> the aws
# extension + credential chain (the server has creds; only place s3:// is needed). It joins the
# `model` view back to the STABLE mdl_key so ad-hoc queries can select by mdl_key, not the internal id.
...
s3_uri <- glue("s3://{sub('^s3://', '', s3_ver)}")
dbExecute(cv, glue("CREATE VIEW model_cell AS
  SELECT m.mdl_key, mc.mdl_id, mc.cell_id, mc.val
  FROM read_parquet('{s3_uri}/serve/model_cell/*/*.parquet', hive_partitioning = true) mc
  JOIN model m USING (mdl_id)"))
dbDisconnect(cv, shutdown = TRUE)
log_info("view DB {viewdb} ({round(file_size(viewdb)/1e3,1)} KB) — tables over {s3_http}, model_cell partitioned by mdl_id (public key = mdl_key)")
```
...
# 3. smoke test — the stable-mdl_key fast-path (titiler resolves ->mdl_id, reads one partition)
...
code <- system2("curl", c("-s","-o","/dev/null","-w","%{http_code}",
  glue("{titiler_v8}/statistics?mdl_key={URLencode(mk, reserved = TRUE)}")), stdout = TRUE)
log_info("titiler-{ver} /statistics?mdl_key={mk} smoke test: HTTP {code}")
}
```
