...
# cell_model lives as LOCAL Parquet on the server (see the viewdb chunk)
...
cell_glob <- if (do_deploy) glue("{srv_cellmodel}/*/*.parquet") else glue("{cell_mc}/*/*.parquet")
dbExecute(cv, glue("CREATE VIEW cell_model AS
  SELECT cm.tile, cm.mdl_id, cm.cell_id, cm.val, cm.val AS value
  FROM read_parquet('{cell_glob}', hive_partitioning = true) cm"))
