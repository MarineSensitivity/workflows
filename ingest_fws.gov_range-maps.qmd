...
```{mermaid}
%%| label: fig-design
%%| fig-cap: "USFWS ranges: ingest the v7-matched species; report pct_marine candidates v7 may have missed"
flowchart LR
  src["usfws current-range shp<br/>(2,205 spp)"] --> er["val = compute_er_score('FWS:code')<br/>EN100 TN50 LC1 (most-protective)"]
  er --> v7["Phase 1: ingest v7 subset<br/>(106 spcodes)"]
  src --> pm["Phase 2: pct_marine<br/>(exact_extract ocean mask) → review"]
  v7 --> ee["cells_from_ranges() vs 0.05° cellid COG"]
  ee --> pq[("dist/dataset=rng_fws/{spcode}.parquet<br/>(mdl_key, cell_id, val) · V2/zstd")]
...
for (i in seq_len(nrow(todo))) {
  r      <- todo[i, ]
  ply_sp <- rng |> filter(spcode == r$spcode)   # no pre-union: cells_from_ranges handles multi-polygon
...
msens::report_table(smry, caption = "rng_fws model_cell surface")
