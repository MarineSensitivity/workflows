...
- [ ] Resolve problematic geometries (see `log_csv`) by converting to raster first and masking to study area raster, rather than intersecting with study area polygon.
...
# dir_rast  <- glue("{dir_bl}/rast") # OLD: vector intersection processing
dir_rast  <- glue("{dir_bl}/botw_rast") # NEW: raster processing with only problematic species from vector intersection processing
botw_gpkg <- glue("{dir_bl}/BOTW_GPKG_2024_2/BOTW_2024_2.gpkg")
# log_txt   <- glue("{dir_rast}/_log 2025-07-23 01:04:17.350045.txt")
log_txt   <- glue("{dir_rast}/_log 2025-07-28 13:35:16.327585.txt")
log_csv   <- glue("{dir_rast}/_ingest_birdlife.org_status.csv")
ds_key    <- "bl"
...
surf2poly = function(obj) {
  # source: https://github.com/r-spatial/sf/issues/2289#issuecomment-1854502398
  
  # obj <- p
  is_multi <- "MULTISURFACE" %in% st_geometry_type(st_geometry(obj))
  ...
  out_geom_txt = obj |> 
    ...
    # gsub(pattern = "CIRCULARSTRING", replacement = 'LINESTRING', fixed = TRUE)
  # TODO: remove extra parenthesis at end?
...
sp_roseate_tern  <- "Sterna dougallii"
roseate_tern_tif <- glue("{dir_rast}/{str_replace_all(sp_roseate_tern, ' ', '_')}.tif")
if (file_exists(log_csv) && !file_exists(roseate_tern_tif)){
  spp <- read_csv(log_csv) |> 
    filter(message %in% c(
      "no valid geometry for species",
...
for (i in 1:length(spp)){ # i = 2 # Acrocephalus_familiaris
  # i = which(spp == "Sterna dougallii")
  # i = which(spp == "Agelasticus cyanopus")
  # i = which(spp == "Actitis hypoleucos")
  ...
  log_info("| INIT | {sp} | {i}/{length(spp)} ({rl_code})")
  if (rl_code == "EX"){
    # TODO: remove EX spp from previous vector intersection processing
    log_info("| SKIP | {sp} | extinct species")
    next()
  }
  ...
  p <- p_spp |> 
    ...
    mutate(
      value = 1L) # placeholder; updated to redlist_code score by merge_models_prep.qmd
  if (nrow(p) == 0){
    log_info("| SKIP | {sp} | no valid presence geometry for species")
    next()
  }

  p <- try(
    surf2poly(p),
    silent = T) # plot(trim(r))
  if (inherits(p, "try-error")){
    log_info("| SKIP | {sp} | error in surf2poly(),...: {p}")
    next()
  }
  # mapView(p)
  
  # OLD: check if geometry intersects with USA ecoregions
  # p <- try(
  #   p |> 
  #     st_make_valid() |> 
  ...
  # NEW: rasterize without vector intersection
  r <- try(
    rasterize(
      p,
      r_cell_r, 
  ...
  if (n_cells == 0){
    log_info("| SKIP | {sp} | rast has no intersecting values")
    next()
  }
  names(r) <- sp
  round(ext(r), 3)
...
d_log <- readLines(log_txt) |> 
  ...
  mutate(
    ...
    has_problem = case_when(
      type == "SKIP" & !(msg %in% c(
        "extinct species", 
        "no valid presence geometry for species", 
        "rast has no intersecting values", 
        "tif already exists")) ~ T,     # T:    443
      .default = F),
    status = case_when(
      tif_created == T ~ "extracted",
      has_problem == T ~ "problem",
      .default = "outside")) |> 
  left_join(
    d_spp |> 
      select(sp = scientific_name, common_name, iucn_redlist = iucn_red_list_category_2024),
    by = "sp")

# only one problem: 
# Hellmayr's Pipit _Anthus hellmayri_ -- error reading geometry

d_log |> 
  select(scientific_name = sp, common_name, iucn_redlist, status, message = msg) |> 
  write_csv(log_csv)
...
# ∆ sp_key 'bl:{spc_rec_id}' -> 'bl:{sis_rec_id}'; add 37 spp with old NAs

dir_rast_0 <- glue("{dir_bl}/rast") # OLD: vector intersection processing
d_rast <- tibble( # 573
  tif = c(
    dir_ls(dir_rast_0, glob = "*.tif"),
    dir_ls(dir_rast, glob = "*.tif"))) |> 
  mutate(
    sp_sci = str_remove(basename(tif), "\\.tif$") |>
      str_replace("_", " ") |>
      str_trim()) |> 
  left_join(
    read_sf(botw_gpkg, "main_BL_HBW_Checklist_V9") |> 
      clean_names(),
  by = join_by(sp_sci == scientific_name)) |> 
  mutate(
    ds_key     = !!ds_key,
    sp_key_old = glue("{ds_key}:{spc_rec_id}"),
    sp_key_new = glue("{ds_key}:{sis_rec_id}"))

sum(duplicated(d_rast$sp_key_old)) # 36
stopifnot(sum(duplicated(d_rast$sp_key_new)) == 0)
# is.na(d_rast$spc_rec_id) |> table()
# FALSE  TRUE 
#   536    37
# is.na(d_rast$sis_rec_id) |> table()
# FALSE 
#   573 

# find model with NA taxa (catchall for previous run using `spc_rec_id` for sp_key)
mdl_seq_na <- tbl(con_sdm, "model") |> 
  filter(
    ds_key == !!ds_key, 
    taxa  == "bl:NA") |> 
  pull(mdl_seq) # mdl_seq_na = 18150
# delete model_cell with mdl_seq_na
if (length(mdl_seq_na) > 0){
  dbExecute(con_sdm, glue("DELETE FROM model_cell WHERE mdl_seq IN ({paste(mdl_seq_na, collapse = ',')})"))
  dbExecute(con_sdm, glue("DELETE FROM model WHERE ds_key = '{ds_key}' AND taxa = 'bl:NA'"))
  dbExecute(con_sdm, glue("DELETE FROM species WHERE ds_key = '{ds_key}' AND taxa = 'bl:NA'"))
}

# register d_spp as temporary table for joining sis_rec_id to species
duckdb_register(con_sdm, "tmp_spp", d_rast)

# update species.taxa & .sp_key to tmp_spp.sp_key_new ----
setdiff(
  d_rast$sp_key_old,
  tbl(con_sdm, "species") |> pull(taxa)) # "bl:NA"
dbExecute(con_sdm, "
  UPDATE species
  SET   taxa   = tmp_spp.sp_key_new,
        sp_key = tmp_spp.sp_key_new
  FROM tmp_spp
  WHERE species.ds_key = tmp_spp.ds_key AND
        species.taxa   = tmp_spp.sp_key_old")
setdiff(
  d_rast$sp_key_new,
  tbl(con_sdm, "species") |> pull(taxa)) # [37] from "bl:NA"

# update model.taxa to tmp_spp.sp_key_new ----
dbExecute(con_sdm, "
  UPDATE model
  SET   taxa = tmp_spp.sp_key_new
  FROM tmp_spp
  WHERE model.ds_key = tmp_spp.ds_key AND
        model.taxa   = tmp_spp.sp_key_old") # 536 rows

duckdb_unregister(con_sdm, "temp_spp")
```

```{r}
#| label: insert_species
...
# DEBUG: quick fix for species with previous "bl:NA"
d_spp <- d_rast |> 
  rename(scientific_name = sp_sci) |> 
  filter(
    !sp_key_new %in% (tbl(con_sdm, "species") |> pull(taxa))) # 37 x 19

r_spp <- rast(d_spp$tif) # 37 x 1
  
for (i in 1:nrow(d_spp)){ # i = 1
  
  sp     <- d_spp$scientific_name[i]
  d_sp   <- d_spp[i,]
  # sp_key <- glue("{ds_key}:{d_sp$spc_rec_id}") # OLD: has NAs
  sp_key <- glue("{ds_key}:{d_sp$sis_rec_id}") # NEW: has no NAs
  r_sp   <- r_spp[[sp]]
