...
msens:
  ...
  workflow_type: merge
  dependency: [ingest_listings, ingest_aquamaps, ingest_birdlife_botw,
               ingest_ch_nmfs, ingest_ch_fws, ingest_rng_fws, ingest_ca_nmfs,
               ingest_turtles_swot_dps, ingest_sdm_gm, ingest_sdm_nc]
...
do_iucn_missing <- F # ingest only missing mammal IUCN ranges (fast)
do_iucn_no_eez <- T # backfill 0-cell rng_iucn entries for species outside US EEZ
```
...
sf_use_s2(F) # prevent invalid geometries; curved geom fix now handles COMPOUNDCURVE/CURVEPOLYGON (previously skipped: Balaena mysticetus, Balaenoptera musculus, Balaenoptera physalus, Eschrichtius robustus, Monodon monoceros, Phocoena phocoena, Delphinus delphis, Phoca vitulina)
...
for (i in 1:length(rng_shps)) {
  ...
  if (file.exists(grp_gpkg)) {
    ...
    # fix curved geometries (MULTISURFACE, COMPOUNDCURVE, CURVEPOLYGON)
    # same approach as ingest_birdlife.org_botw.qmd surf2poly()
    geom_types <- st_geometry_type(a) |> as.character() |> unique()
    curved_types <- c(
      "MULTISURFACE",
      "COMPOUNDCURVE",
      "CURVEPOLYGON",
    ...
    if (any(curved_types %in% geom_types)) {
      log_info(
        "| FIX | {grp} | converting curved geometries: {paste(intersect(curved_types, geom_types), collapse = ', ')}"
      )
      out_geom_txt <- a |>
        sf::st_geometry() |>
        sf::st_as_text() |>
      ...
      a <- a |>
        sf::st_set_geometry(
          value = sf::st_as_sfc(out_geom_txt, crs = sf::st_crs(a))
        )
      idx_line <- which(sf::st_geometry_type(a) == "LINESTRING")
      if (length(idx_line) > 0) {
        a_ok <- a[-idx_line, ]
        a_line <- a |>
          dplyr::slice(idx_line) |>
      ...
      a <- a |>
        ...
        sf::st_make_valid()
      log_info(
        "| FIX | {grp} | curved geometry conversion complete, {sum(!st_is_valid(a))} still invalid"
      )
    }
...
# left_join so species with IUCN range outside US EEZ (no tif) still get
# model + taxon_model entries with 0 cells — merge logic then clips AquaMaps
# to empty mask → 0 merged cells → is_ok=FALSE via "no distribution in PRA"
d_spp <- d_gpkg |>
  left_join(
    d_tif,
    by = join_by(sci_name)
...
for (i in 2:nrow(d_spp)) {
  ...
  d_model <- tibble(
    ...
    description = glue(
      ...
      if (!has_tif) " [range outside US EEZ — 0 cells]" else ""
    )
  )
  ...
  d_species <- tibble(
    ...
    common_name_dataset = NA_character_,
  ...
  # append: taxon_model ----
  d_taxon_model <- tibble(
    taxon_id = d_sp$taxon_id,
    ds_key = ds_key,
    mdl_seq = mdl_seq
  )
  dbWriteTable(con_sdm, "taxon_model", d_taxon_model, append = TRUE)

  # append: model_cell (only if range overlaps US EEZ) ----
  if (has_tif) {
    sp_tif <- d_sp$tif
  ...
  # species with no tif get model + taxon_model but 0 model_cell rows
}
...
### Ingest missing mammal IUCN range maps

Quickly ingest IUCN range maps for mammal species that have `is_ok=TRUE` in the
taxon table but are missing a `rng_iucn` entry. Uses the newer MAMMALS_PART1/PART2
shapefiles which have clean Polygon geometry (no curved geometry issues).

Set `do_iucn_missing <- TRUE` in the flags chunk to run.

```{r}
#| label: setup_iucn_missing
#| eval: !expr do_iucn_missing

ds_key <- "rng_iucn"
dir_iucn_raw <- glue("{dir_data}/raw/iucnredlist.org")
dir_iucn_derived <- glue("{dir_data}/derived/iucnredlist.org")
default_cell_value <- 1L
sf_use_s2(F)
...
# mammals with is_ok=TRUE but no rng_iucn
d_have_iucn <- tbl(con_sdm, "taxon_model") |>
  filter(ds_key == "rng_iucn") |>
  select(taxon_id) |>
  collect()

d_missing_mammals <- d_taxon |>
  filter(
    is_ok,
    sp_cat == "mammal",
    !taxon_id %in% d_have_iucn$taxon_id
  )

message(glue(
  "{nrow(d_missing_mammals)} mammals with is_ok=TRUE missing rng_iucn"
))

# find MAMMALS_PART* shapefiles
mammal_shps <- list.files(
  dir_iucn_raw,
  pattern = "MAMMALS_PART.*\\.shp$",
  recursive = TRUE,
  full.names = TRUE
)
message(glue(
  "mammal shapefiles: {paste(basename(mammal_shps), collapse = ', ')}"
))
```

```{r}
#| label: extract_insert_iucn_missing
#| eval: !expr do_iucn_missing

if (nrow(d_missing_mammals) > 0 && length(mammal_shps) > 0) {
  # read and combine MAMMALS_PART* shapefiles
  a_all <- bind_rows(lapply(mammal_shps, read_sf))

  # fix curved geometries if present (safety net)
  geom_types <- st_geometry_type(a_all) |> as.character() |> unique()
  curved_types <- c(
    "MULTISURFACE",
    "COMPOUNDCURVE",
    "CURVEPOLYGON",
  ...
  if (any(curved_types %in% geom_types)) {
    message("fixing curved geometries...")
    out_geom_txt <- a_all |>
      sf::st_geometry() |>
      sf::st_as_text() |>
      gsub(pattern = "MULTISURFACE (", replacement = "", fixed = TRUE) |>
    ...
    a_all <- a_all |>
      sf::st_set_geometry(
        value = sf::st_as_sfc(out_geom_txt, crs = sf::st_crs(a_all))
      )
    idx_line <- which(sf::st_geometry_type(a_all) == "LINESTRING")
    if (length(idx_line) > 0) {
      a_ok <- a_all[-idx_line, ]
      a_line <- a_all |>
        dplyr::slice(idx_line) |>
  ...
  if (any(!st_is_valid(a_all))) {
    a_all <- st_make_valid(a_all)
  }

  # filter to marine species matching missing mammals
  x <- a_all |>
    rename(iucn_id = id_no) |>
    filter(
      str_to_lower(marine) %in% c("true", "t"),
      sci_name %in% d_missing_mammals$scientific_name
    ) |>
    inner_join(
      d_missing_mammals |>
        select(taxon_id, scientific_name),
    ...
    group_by(iucn_id, taxon_id, sci_name, category) |>
    summarize(
      yrcompiled = max(yrcompiled, na.rm = TRUE),
      n_shp_rows = n(),
      .groups = "drop"
    )
  ...
  message(glue("found {nrow(x)} missing mammals in shapefiles"))

  grp <- "MAMMALS_MISSING"
  dir.create(
    glue("{dir_iucn_derived}/{grp}"),
    recursive = TRUE,
    showWarnings = FALSE
  )
  ...
  for (j in seq_len(nrow(x))) {
    sp <- x$sci_name[j]
    sp_key <- glue("{ds_key}:{sp}")
    sp_tif <- glue("{dir_iucn_derived}/{grp}/{sp}.tif")

    message(glue("  [{j}/{nrow(x)}] rasterizing {sp}..."))
    ...
    if (inherits(r, "try-error")) {
      message(glue("  SKIP: rasterize error for {sp}: {r}"))
      next
    }

    n_notna <- global(r, "notNA") |> as.integer()
    has_eez_cells <- n_notna > 0

    if (has_eez_cells) {
      writeRaster(r, filename = sp_tif, overwrite = TRUE)
    }

    # delete existing rng_iucn if present (shouldn't be, but safety) ----
    mdl_seqs_old <- tbl(con_sdm, "model") |>
      filter(ds_key == !!ds_key, taxa == !!sp_key) |>
      pull(mdl_seq)
    if (length(mdl_seqs_old) > 0) {
      dbExecute(
        con_sdm,
        glue(
          "DELETE FROM taxon_model WHERE ds_key = '{ds_key}' AND taxon_id = {x$taxon_id[j]}"
        )
      )
      dbExecute(
        con_sdm,
        glue(
          "DELETE FROM model WHERE ds_key = '{ds_key}' AND taxa = '{sp_key}'"
        )
      )
      dbExecute(
        con_sdm,
      ...
      dbExecute(
        con_sdm,
        glue(
          "DELETE FROM model_cell WHERE mdl_seq IN ({paste(mdl_seqs_old, collapse = ',')})"
        )
      )
    }

    # insert: model (always — even with 0 EEZ cells, so merge clips AquaMaps) ----
    d_model <- tibble(
      ds_key = ds_key,
      taxa = sp_key,
      time_period = "2025",
      ...
      description = glue(
        "IUCN Red List (2025-2) range map for {sp}, interpolated to 0.05° resolution",
        if (!has_eez_cells) " [range outside US EEZ — 0 cells]" else ""
      )
    )
    dbWriteTable(con_sdm, "model", d_model, append = TRUE)
    ...
    # insert: taxon_model (always — links species to rng_iucn even with 0 cells) ----
    d_taxon_model <- tibble(
      taxon_id = d_sp$taxon_id,
      ds_key = ds_key,
      mdl_seq = mdl_seq
    ...
    # insert: model_cell (only if range overlaps US EEZ) ----
    if (has_eez_cells) {
      r_sp <- rast(sp_tif, lyrs = sp)
      d_mdl_cell <- as.data.frame(r_sp, cells = TRUE, na.rm = TRUE) |>
        tibble() |>
        select(cell_id = cell) |>
        mutate(
          mdl_seq = mdl_seq,
      ...
      dbWriteTable(con_sdm, "model_cell", d_mdl_cell, append = TRUE)

      rl_value <- switch(
        as.character(d_sp$category),
        "CR" = 50L,
        "EN" = 25L,
        "VU" = 5L,
        "NT" = 2L,
      ...
      dbExecute(
        con_sdm,
        glue(
          "UPDATE model_cell SET value = {rl_value} WHERE mdl_seq = {mdl_seq}"
        )
      )

      message(glue(
        "  OK: {sp} — mdl_seq={mdl_seq}, {n_notna} cells, ",
        "category={d_sp$category}, value={rl_value}"
      ))
    } else {
      message(glue(
        "  OK (0 EEZ cells): {sp} — mdl_seq={mdl_seq}, ",
        "range outside US EEZ, model+taxon_model inserted"
      ))
    }
  }
}
...
### Backfill 0-cell rng_iucn entries for species outside US EEZ

One-time backfill: species whose IUCN range was rasterized during ingestion but
produced 0 cells in the US EEZ were previously skipped entirely. This chunk
reads the log-derived CSV and inserts `model` + `species` + `taxon_model` rows
(with 0 `model_cell` rows) so the merge logic clips their AquaMaps predictions
to an empty mask → 0 merged cells → `is_ok=FALSE`.

Set `do_iucn_no_eez <- FALSE` after running once.

```{r}
#| label: backfill_iucn_no_eez
#| eval: !expr do_iucn_no_eez

ds_key <- "rng_iucn"

d_no_eez <- read_csv(
  here("data/iucn_no_eez_species.csv"),
  show_col_types = FALSE
)

d_taxon <- tbl(con_sdm, "taxon") |> collect()

# species already with rng_iucn in database (skip those)
d_have_iucn <- tbl(con_sdm, "taxon_model") |>
  filter(ds_key == "rng_iucn") |>
  select(taxon_id) |>
  collect()

# match CSV against taxon table, exclude species that already have rng_iucn
d_backfill <- d_taxon |>
  filter(
    scientific_name %in% d_no_eez$scientific_name,
    !taxon_id %in% d_have_iucn$taxon_id
  ) |>
  select(taxon_id, scientific_name, sp_cat, redlist_code)

message(glue(
  "{nrow(d_backfill)} species to backfill with 0-cell rng_iucn entries ",
  "(out of {nrow(d_no_eez)} in CSV, {nrow(d_have_iucn)} already have rng_iucn)"
))
# 2,247 species to backfill with 0-cell rng_iucn entries (out of 2,247 in CSV, 1,519 already have rng_iucn)
# done: 2247 species now have 0-cell rng_iucn entries

for (i in seq_len(nrow(d_backfill))) {
  sp <- d_backfill$scientific_name[i]
  sp_key <- glue("{ds_key}:{sp}")
  tid <- d_backfill$taxon_id[i]
  ...
  d_model <- tibble(
    ...
    mdl_type = "binary",
    description = glue(
      "IUCN Red List (2025-2) range map for {sp}, ",
      "interpolated to 0.05° resolution ",
      "[range outside US EEZ — 0 cells]"
    )
  )
  dbWriteTable(con_sdm, "model", d_model, append = TRUE)
  ...
  d_species <- tibble(
    ...
    worms_is_marine = NA,
    worms_is_extinct = NA,
    redlist_code = d_backfill$redlist_code[i],
    redlist_year = NA_integer_,
    sp_cat = d_backfill$sp_cat[i]
  )
  dbWriteTable(con_sdm, "species", d_species, append = TRUE)

  # insert: taxon_model (0 cells in model_cell) ----
  d_taxon_model <- tibble(
    taxon_id = tid,
    ds_key = ds_key,
    mdl_seq = mdl_seq
  ...
  if (i %% 100 == 0 || i == nrow(d_backfill)) {
    message(glue("  backfilled {i}/{nrow(d_backfill)}"))
  }
}

message(glue(
  "done: {nrow(d_backfill)} species now have 0-cell rng_iucn entries"
))
```
