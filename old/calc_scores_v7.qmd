...
msens:
  target_name: calc_scores
  workflow_type: score
  dependency: [merge_models]
  output: data/manifests/calc_scores.json
format:
  html:
    code-fold: true
    code-tools: true
...
do_stac <- T # generate static STAC catalog (sdm extension) from the sdm db
...
# backfill `cell` with modeled-but-missing cells. Range-map presence cells in areas
# without Bio-Oracle environmental data were excluded from `cell` during ingest, yet
# they carry model_cell values. Because zone_taxon inner-joins `cell` for area_km2,
# those species would be dropped from per-zone species tables. Add the cells with their
# geometric area (env columns left NULL) so every modeled cell is represented.
missing_cells <- dbGetQuery(
  con_sdm,
  "SELECT DISTINCT cell_id FROM model_cell WHERE cell_id NOT IN (SELECT cell_id FROM cell)")
if (nrow(missing_cells) > 0) {
  if (!exists("r_cell")) r_cell <- rast(cell_tif)
  area_all <- terra::values(terra::cellSize(r_cell[[1]], unit = "km", mask = FALSE))[, 1]
  d_add <- tibble(
    cell_id  = as.integer(missing_cells$cell_id),
    area_km2 = as.numeric(area_all[missing_cells$cell_id]))
  stopifnot(!any(is.na(d_add$area_km2)))
  duckdb_register(con_sdm, "tmp_add_cell", d_add)
  dbExecute(
    con_sdm,
    "INSERT INTO cell (cell_id, area_km2) SELECT cell_id, area_km2 FROM tmp_add_cell")
  duckdb_unregister(con_sdm, "tmp_add_cell")
  message(glue("backfilled {nrow(d_add)} modeled-but-missing cells into `cell`"))
}
```
...
### Calculate ProgramArea + Subregion metrics contributing to score based on cell metrics

The same weighted-mean SQL works for any zone (program area, subregion,
ecoregion) because it joins through `zone_cell`. We aggregate to both
program-area and subregion zones in one pass so the app can render a
default flower plot for "All USA" / Alaska / Pacific / Gulf of America
without computing on the fly. Note: the `pct_area` weighting in the
next chunk only applies to program-area zones; subregion zones get the
raw weighted mean.

```{r}
#| label: cell_metrics_to_zone_metrics
#| eval: !expr do_metrics

# get zone_seqs for program areas + subregions
z_seqs <- tbl(con_sdm, "zone") |>
  filter(tbl %in% c(!!tbl_pra, !!tbl_sr)) |>
  pull(zone_seq)

n_pra <- tbl(con_sdm, "zone") |> filter(tbl == !!tbl_pra) |> tally() |> pull(n)
n_sr  <- tbl(con_sdm, "zone") |> filter(tbl == !!tbl_sr ) |> tally() |> pull(n)
message(glue(
  "Calculating metrics for {length(z_seqs)} zones ",
  "({n_pra} program areas + {n_sr} subregions)..."))
...
# verify (program areas + subregions)
d_ck <- tbl(con_sdm, "metric") |>
  ...
  left_join(
    tbl(con_sdm, "zone") |>
      filter(tbl %in% c(!!tbl_pra, !!tbl_sr)) |>
      select(zone_seq, zone_tbl = tbl, zone_value = value),
    by = "zone_seq"
  ) |>
  filter(!is.na(zone_value)) |>
  collect()
...
### Apply pct_area weighting to ProgramArea component metrics
...
```{r}
#| label: apply_pctarea_to_programarea_components
#| eval: !expr do_metrics
...
## Add "Full study area" zone

A subregion-level zone (`subregion_key = 'FULL'`) covering **every** cell, so the apps'
default study-area-wide species table and flower plot reflect all valid species — not only
those within Program Areas (the "USA" subregion). Created before `zone_taxon` so it is picked
up alongside the other subregions; its `zone_metric` is the coverage-weighted mean over all
cells. (Folds in the former `dev/add_full_zone.R`.)

```{r}
#| label: add_full_studyarea_zone
#| eval: !expr do_metrics

full_seq <- tbl(con_sdm, "zone") |>
  filter(tbl == !!tbl_sr, fld == "subregion_key", value == "FULL") |>
  pull(zone_seq)
if (length(full_seq) == 0) {
  dbExecute(con_sdm, glue(
...
# zone_metric = coverage-weighted mean of every metric over all cells
...
dbExecute(con_sdm, glue("
  ...
  GROUP BY zc.zone_seq, cm.metric_seq"))
message(glue("FULL study-area zone ready (zone_seq = {full_seq})"))
```
...
d <- tbl(con_sdm, "zone") |>
  filter(
    tbl %in% c(tbl_sr, tbl_pra, tbl_er)
  ) |>
...
Emit a static [STAC](https://stacspec.org/) catalog (using the [sdm extension](https://github.com/MarineSensitivity/stac-sdm)) describing this version's SDM products plus the live DuckDB-SQL TiTiler endpoint. The tree is written to `{dir_derived}/stac` (shared across versions) and synced to the server by the deploy chunk, where it is served at `https://file.marinesensitivity.org/stac/`.

```{r}
#| label: stac_catalog
#| eval: !expr do_stac

# generators live in the msens package (msens/R/stac.R); source the sibling repo
# until the package is reinstalled with them
librarian::shelf(jsonlite, base64enc, quiet = T)
suppressWarnings(try(library(msens), silent = TRUE))
if (!exists("stac_build")) {
  source(here("../msens/R/stac.R"))
}

stac_dir <- glue("{dir_derived}/stac") # shared (not version-specific); served at /stac
nc_csv <- here("data/nc_models.csv") # NCCOS seasonal source (not in the duckdb)

# the db was shut down after import_parquet — open a fresh read-only connection
con_stac <- dbConnect(duckdb(dbdir = sdm_db, read_only = TRUE))
stac_build(
  version = ver,
  dir_out = stac_dir,
  nc_csv  = if (file_exists(nc_csv)) nc_csv else NULL,
  con     = con_stac)
dbDisconnect(con_stac, shutdown = TRUE)

message(glue(
  "STAC catalog: {stac_dir}/catalog.json  ->  ",
