...
`sdm_v3.duckdb`. This notebook runs **BEFORE** `merge_models.qmd`.
...
d_listing <- d_nw |>
  select(worms_id, nmfs_esa, is_mmpa, common_name) |>
  full_join(
    d_f_bw |>
      select(worms_id, botw_id, fws_esa, is_bcc,
             fws_common_name = common_name),
    by = "worms_id",
    na_matches = "never"
  ) |>
  left_join(
  ...
  mutate(
    ...
    # nmfs common_name takes priority over fws
    common_name = coalesce(common_name, fws_common_name)
  ) |>
  select(-fws_common_name)
...
message(glue(
  "Wrote {nrow(d_listing_out)} rows to listing table in sdm_v3.duckdb"
))
