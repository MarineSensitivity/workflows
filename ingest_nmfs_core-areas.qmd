...
from scratch, we copy cell_ids from the existing IUCN range map and assign
maximum weighting (value = 100).
...
Copy cell_ids from the existing IUCN range map for Rice's whale.

```{r}
#| label: add_rices_whale
# rice's whale parameters
sp_sci       <- "Balaenoptera ricei"
sp_cmn       <- "Rice's whale"
worms_id     <- 1576133L
# dynamically look up the current rng_iucn mdl_seq (not hardcoded)
...
# delete existing ca_nmfs entries for this taxon (idempotent re-runs)
dbExecute(con_sdm, glue(
...
# verify taxon_model entry
d_tm_check <- tbl(con_sdm, "taxon_model") |>
  filter(ds_key == !!ds_key) |>
  collect()
message(glue("taxon_model ca_nmfs rows: {nrow(d_tm_check)}"))
stopifnot(nrow(d_tm_check) == 1)
```
