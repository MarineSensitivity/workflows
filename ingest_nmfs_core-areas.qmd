---
title: "Ingest NMFS Core Distribution Areas → global 0.05° cells"
msens:
  target_name: ingest_ca_nmfs
  workflow_type: ingest
  dependency: [build_cell_grid]
  output: data/manifests/ingest_ca_nmfs.json
  dataset: {ds_key: ca_nmfs, response_type: range, source_authority: NOAA NMFS, temporal_interval: static, native_format: vector}
editor_options:
  chunk_output_type: console
---

NMFS **Core Distribution Areas** onto the **global 0.05° cell grid**. Currently one:
**Rice's whale** (*Balaenoptera ricei*), whose designated core area gets maximum
weighting (`value = 100`). `mdl_key = "ca_nmfs|{sp_id}"`. Same vector→cells pattern
(`msens::cells_from_ranges()`) as the other range datasets; add rows to `areas` for
future core areas.

## Setup + rasterize

```{r}
#| label: ingest

librarian::shelf(
  arrow, DBI, dplyr, duckdb, fs, glue, here, jsonlite, logger, readr, sf, terra,
  tibble, MarineSensitivity/msens, quiet = T)
source(here("libs/paths.R"))   # dir_raw, dir_big_v, cellid_tif, ver
source(here("libs/vars.R"))    # redo_ingest
options(readr.show_col_types = F)

ds_key   <- "ca_nmfs"
dir_ca   <- glue("{dir_raw}/fisheries.noaa.gov/core-areas")
dir_dist <- glue("{dir_big_v}/marine-atlas/dist/dataset={ds_key}")
manifest <- here("data/manifests/ingest_ca_nmfs.json")
dir_create(c(dir_dist, path_dir(manifest)))
stopifnot("run build_cell_grid.qmd first" = file_exists(cellid_tif))

# core-area shapefile(s) -> species; extend this table as NMFS adds core areas
areas <- tribble(
  ~sp_id,               ~sci_name,             ~shp,
  "Balaenoptera_ricei", "Balaenoptera ricei",
    "shapefile_Rices_whale_core_distribution_area_Jun19_SERO/shapefile_Rices_whale_core_distribution_area_Jun19_SERO.shp") |>
  mutate(mdl_key = mdl_key_raw(ds_key, sp_id))
write_csv(areas |> select(mdl_key, sp_id, sci_name), glue("{dir_dist}/../model_{ds_key}.csv"))

if (redo_ingest && dir_exists(dir_dist)) { dir_delete(dir_dist); dir_create(dir_dist) }
for (i in seq_len(nrow(areas))) {
  a <- areas[i, ]; out_pq <- fs::path(dir_dist, a$sp_id, ext = "parquet")
  if (file_exists(out_pq)) { log_info("{a$sp_id}: skip"); next }
  ply <- st_read(glue("{dir_ca}/{a$shp}"), quiet = TRUE)
  d   <- cells_from_ranges(ply, cellid_tif, value = 100)     # core area = max weight
  if (nrow(d) == 0) { log_warn("{a$sp_id}: no cells"); next }
  write_parquet(tibble(mdl_key = a$mdl_key, cell_id = d$cell_id, value = d$value), out_pq)
  log_info("{a$sp_id}: {nrow(d)} cells")
}
```

## Verify + manifest

```{r}
#| label: verify

pq  <- dir_ls(dir_dist, glob = "*.parquet")
con <- dbConnect(duckdb())
smry <- dbGetQuery(con, glue(
  "SELECT count(DISTINCT mdl_key) n_models, count(*) n_cells, max(value) v_max
     FROM read_parquet('{dir_dist}/*.parquet')"))
dbDisconnect(con, shutdown = TRUE)
knitr::kable(smry)

write_json(
  list(target = "ingest_ca_nmfs", ds_key = ds_key, ver = ver,
       n_models = nrow(areas), n_parquet = length(pq),
       dir_dist = dir_dist, built = as.character(Sys.time())),
  manifest, auto_unbox = TRUE, pretty = TRUE)
```
