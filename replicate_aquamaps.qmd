...
librarian::shelf(
  ...
  leaflet, leaflet.extras, librarian, mapview, ggplot2, listviewer, scales,
...
get_hcaf_raster <- function(con_dd) {
...
replicate_sp_raster <- function(sp_key, con_dd, r_h = NULL) {
...
validate_aquamaps_species <- function(sp_keys, con_dd) {
  ...
  for (sp_key in sp_keys) {
    ...
    tryCatch({
      # Get original raster
      r_sp_old <- get_species_raster(sp_key, con_dd)
      
      # Replicate raster
      r_sp_new <- replicate_sp_raster(sp_key, con_dd, r_h)
...
# Get original raster
r_sp_old <- get_species_raster(test_sp_key, con_dd)

# Replicate raster
r_sp_new <- replicate_sp_raster(test_sp_key, con_dd)
...
max_species <- 100  # Set to NULL for all species
...
# Validate species
results <- validate_aquamaps_species(all_sp_keys, con_dd)
...
if (sum(!results$r_matches, na.rm=TRUE) > 0) {
  ...
  for (sp in sample_sp) {
    ...
    r_old <- get_species_raster(sp, con_dd)
    r_new <- replicate_sp_raster(sp, con_dd)
    
    # Create comparison map
    cat(glue("\nExample non-matching species: {sp}\n"))
    print(compare_sp(r_old, r_new))
  }
}
```
