...
# Validate species
results <- validate_aquamaps_species(sp_keys, con_dd, dir_cache = dir_cache, verbosity = 1, redo = F)
...
```{r}
#| label: bio-oracle_layers
#| eval: false

librarian::shelf(
  bio-oracle/biooracler, janitor, terra,
  quiet = T)
...
if (!all(file.exists(bo_ds_csv, bo_ds_v_csv))) {
...
# download bio-oracle layers for depth_mean|min|max: bathymetry
dir_bo <- '~/My Drive/projects/msens/data/raw/bio-oracle.org'
r_nc <- glue("{dir_bo}/bathymetry.nc")
