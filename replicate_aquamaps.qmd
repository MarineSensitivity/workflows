...
ramp_env <- function(v, p){ #}, digits = 10) { # 10: 1,174 8: 1,178 6: 1,178 5:1,199 4:1,405
...
replicate_sp_raster <- function(sp_key, con_dd, r_hcaf = NULL, dir_cache = here("data/replicate_aquamaps"), redo = F) {
  ...
  message(glue("is_surface: {is_surface}"))
  var_lyr <- list(
  ...
  for (var in vars_yes) { # var = "depth"
    ...
    if (var == "depth"){
      ...
      if (sp_info$taxa$class == "Mammalia"){
        ...
        message("Normal: max(depth_mean, depth_min, depth_max)")
...
compare_rasters <- function(r_old, r_new, return = "boolean", tolerance = 0.02) {
  ...
  d_cmp <- values(r_cmp, na.rm = F, dataframe = T) |> 
    ...
    mutate(
      ...
      prod_depthmax   = round(depth_tx_max * ice_con_tx * prim_prod_tx * salinity_tx * temp_tx, 2),
      prod_depthmean  = round(depth_tx     * ice_con_tx * prim_prod_tx * salinity_tx * temp_tx, 2),
      # prod          = round(
    ...
    # filter(!prod_eq_old) |> 
    relocate(dif_abs, dif, prod_nodepth_eq_old, prod_depthmax_eq_old, prod_depthmean_eq_old) |> 
...
validate_aquamaps_species <- function(sp_keys, con_dd, dir_cache = here("data/replicate_aquamaps"), verbose = F) {
  ...
  for (i in 1:length(sp_keys)) { # i = 1
  
    sp_key <- sp_keys[i]
    if (verbose)
      log_info("{sprintf('%04d', i)} of {length(sp_keys)}: {sp_key}...\n") # sp_key = "ITS-206881"
...
if (!matches){
  ...
  # r_env <- get_hcaf_raster() |>
  #   crop(r_sp_new)
  # r_sp_new[r_env[["cell_id"]] == 239328]
  # r_env[["depth_mean"]][r_env[["cell_id"]] == 239328]
  #   names() |> sort()
  #   mask(r_sp_new) |> 
  #   subset("cell_id", "depth_mean", "depth_min", "depth_max")
  # r_sp_new
  ...
  # |>
    # glimpse()
    # View()
  # ITS-Mam-180527 fin whale cid=239328 old 0.09 -> new 0.98
...
# INFO [2025-05-20 01:18:36] 0101 of 9927:
# INFO [2025-05-20 01:23:26] 0121 of 9927
# 20/5 = 4 per min
# 9927 / 4 / 60 = 41.4 hrs
# INFO [2025-05-20 01:26:13] 0137 of 9927
