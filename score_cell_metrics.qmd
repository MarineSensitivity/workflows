...
1. **`extrisk_{spcat}`** = `SUM(er × val / 100)` over valid species in the category, where the
   weight `er` depends on the taxon's `er_mode` (score_zones): the governing `er_score` (scalar,
   MMPA/MBTA floors baked in) for ordinary taxa; `100` for the sea turtles, whose merged `val` is
   already ER × suitability (`premultiplied`); the **per-cell** ER from `model_cell_er` for the
   NMFS DPS species (`cell`), whose merged `val` is the suitability alone.
...
flowchart LR
  mt["model_cell × taxon<br/>(er_score | 100 | model_cell_er by er_mode; sp_cat, is_marine)"] --> ex["extrisk_{spcat}<br/>Σ er·val/100"]
...
# the ER weight per (taxon, cell) by er_mode -- see score_zones; a 'cell' taxon without its per-cell
# ER would be weighted NULL (dropped from the sum), so the join is asserted complete first
stopifnot("run score_zones first (taxon.er_mode / model_cell_er missing)" =
            "er_mode" %in% dbListFields(con, "taxon") && "model_cell_er" %in% dbListTables(con))
...
stopifnot("er_mode = 'cell' taxa have scored cells without a per-cell ER" = n_gap == 0)
