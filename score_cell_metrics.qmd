...
1. **`extrisk_{spcat}`** = `SUM(er_score × value / 100)` over valid species in the category
...
dbExecute(con, "CREATE TABLE IF NOT EXISTS metric (metric_seq INTEGER PRIMARY KEY, metric_key VARCHAR, description VARCHAR)")
dbExecute(con, "CREATE TABLE IF NOT EXISTS cell_metric (cell_id INTEGER, metric_seq INTEGER, value DOUBLE)")
dbExecute(con, "CREATE TABLE IF NOT EXISTS zone_metric (zone_seq INTEGER, metric_seq INTEGER, value DOUBLE)")
...
# 7 v7 scoring categories (reptile folds into 'other')
sp_cats <- c("bird","coral","fish","invertebrate","mammal","other","turtle")

# species set: default = all US-valid; SCORE_V7COMMON=1 -> restrict to v7's scored set
# (in_v7) for an apples-to-apples v7 comparison that isolates grid/algorithm from the
# v8 species-set expansion
...
for (cat in sp_cats) {
  ...
  dbExecute(con, glue("INSERT INTO cell_metric (cell_id, metric_seq, value)
    SELECT mc.cell_id, {s}, ROUND(SUM(
        (CASE WHEN t.is_er_spatial THEN 100 ELSE t.er_score END) * mc.value) / 100.0, 2) AS value
    FROM model_cell mc JOIN taxon t ON mc.mdl_key = t.ms_merge_key
    WHERE {taxa_where}
      AND (CASE WHEN t.sp_cat = 'reptile' THEN 'other' ELSE t.sp_cat END) = '{cat}'
    GROUP BY mc.cell_id"))
}
...
rescale_ecoregion <- function(base_key) {
  ...
  for (fx in list(c(smin,"min"), c(smax,"max")))
    dbExecute(con, glue("INSERT INTO zone_metric (zone_seq, metric_seq, value)
      SELECT z.zone_seq, {fx[1]}, {fx[2]}(cm.value)
  ...
  dbExecute(con, glue("INSERT INTO cell_metric (cell_id, metric_seq, value)
    ...
    mm AS (SELECT mn.zone_seq, mn.value min_v, mx.value max_v
           FROM (SELECT zone_seq,value FROM zone_metric WHERE metric_seq={smin}) mn
           JOIN (SELECT zone_seq,value FROM zone_metric WHERE metric_seq={smax}) mx USING(zone_seq))
    SELECT cm.cell_id, {srs},
      SUM((cm.value - mm.min_v)/(mm.max_v - mm.min_v) * (ce.norm_pct/100.0)) * 100 AS value
...
dbExecute(con, glue("INSERT INTO cell_metric (cell_id, metric_seq, value)
  SELECT p.cell_id, {sp}, p.value FROM tmp_pp_df p JOIN cell c ON p.cell_id = c.cell_id WHERE c.in_usa"))
...
dbExecute(con, glue("INSERT INTO cell_metric (cell_id, metric_seq, value)
  SELECT cm.cell_id, {sc}, ROUND(AVG(cm.value))
  FROM cell_metric cm JOIN metric m USING(metric_seq)
  WHERE m.metric_key IN ({in_list}) GROUP BY cm.cell_id"))

smry <- dbGetQuery(con, glue("SELECT count(*) n_cells, round(min(value),1) v_min, round(max(value),1) v_max,
  round(avg(value),1) v_avg FROM cell_metric WHERE metric_seq={sc}"))
knitr::kable(smry)
