...
```{mermaid}
%%| label: fig-design
%%| fig-cap: "SWOT ranges + NMFS DPS boundaries → cell-level EN/TN turtle ranges on the global 0.05° grid"
flowchart LR
  swot["SWOT global distribution<br/>(6 species)"] --> rng["cells_from_ranges(val=TN=50)"]
  dps["NMFS DPS boundaries<br/>(Endangered sub-pops)"] --> en["cells_from_ranges(val=EN=100)"]
  rng --> comb["cell-level combine<br/>EN inside DPS, else TN"]
  en --> comb
  comb --> pq[("dist/dataset=rng_turtle_swot_dps/{code}.parquet<br/>(mdl_key, cell_id, val) · V2/zstd")]
