...
# model_cell = merged US model (mdl_key, cell_id, value); materialize for scoring joins
dbExecute(con, glue("CREATE OR REPLACE TABLE model_cell AS
  SELECT mdl_key, cell_id, value FROM read_parquet('{merged}/**/*.parquet')"))
...
# is_er_spatial = turtles (SWOT+DPS ER baked into the merged cell value -> er_score=100
...
dbExecute(con, "CREATE OR REPLACE TABLE v7_cat AS
  SELECT taxon_authority, CAST(CAST(taxon_id AS BIGINT) AS VARCHAR) AS taxon_id, any_value(sp_cat) AS sp_cat
...
for (zs in zone_specs) {
  ...
  for (i in seq_len(nrow(ply))) {
    ...
    zone_rows[[length(zone_rows) + 1]] <- tibble(
      zone_seq = zseq, tbl = zs$tbl, fld = zs$fld, value = as.character(zval))
    # coverage fraction of this zone polygon over each cell -> pct_covered (0-100)
    df <- exact_extract(r_cell, ply[i, ], progress = FALSE)[[1]]
